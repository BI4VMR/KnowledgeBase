...
<!-- TODO
ADB是一个CS架构的程序，计算机作为客户端5037Android设备和ADB命令作为客户端，，我们可以通过USB线缆或TCP/IP网络使用ADB。
-->
...
> 🚩 提示
>
> `-s <设备标识符>` 选项必须紧接着 `adb` 命令书写，如果书写在其他位置会出现错误。
...
有时我们希望使用ADB Shell执行命令语句，但不进入ADB Shell中，此时可以使用 `adb shell "<命令语句...>"` 命令。
...
这是ADB Shell的非交互式使用方法，我们可以在编写脚本文件等场景中使用。
