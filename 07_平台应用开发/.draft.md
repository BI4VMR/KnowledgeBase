...
# 尺寸计量单位
## 密度无关像素
密度无关像素(Density Independent Pixel, DIP)也被称为DP，是Android中特有的尺寸单位，它能够根据屏幕的像素密度(DPI)自动缩放，以适配不同种类的终端。

像素密度(DPI)是屏幕的物理属性，Android根据DPI范围定义了若干缩放倍率，详见下文表格：

<div align="center">

| DPI范围 | DPI等级  | "1dp"所对应的"px"值 |
| :-----: | :------: | :-----------------: |
|  0-120  |  `ldpi`  |        0.75         |
| 120-160 |  `mdpi`  |          1          |
| 160-240 |  `hdpi`  |         1.5         |
...
我们将某个控件的宽高分别设为 `100dp * 200 dp` 后，如果界面显示在DPI等级为 `mdpi` 的屏幕上，缩放倍率为"1"，实际尺寸为 `100dp * 200 dp`；如果界面显示在DPI等级为 `xxdpi` 的屏幕上，缩放倍率为"3"，实际尺寸为 `100dp * 200 dp` 。
