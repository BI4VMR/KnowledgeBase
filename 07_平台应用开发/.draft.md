...
Jetpack Compose的核心组件是必选的；调试工具可以在开发阶段为开发者提供预览等功能，因此我们通常只会为开发阶段的BuildType声明这些依赖项；测试工具则提供了对于JUnit的支持，如果我们不需要编写测试用例，可以不声明这些依赖项。
