...
> Android - 辅助工具 - 实用工具 - ADB

# 简介
ADB的全称为Android Debug Bridge，是Android设备的调试工具，主要包括 `adb` 和 `recovery` 两个命令，我们可以通过它们查看日志、向设备中传输文件、刷写系统镜像等。

ADB是一个CS架构的软件，以PC作为服务端、Android设备作为客户端，我们可以通过USB线缆或TCP/IP网络使用ADB。

ADB包含以下三个部分：

🔷 ADB Server

ADB Server运行在PC上，默认使用TCP 5037端口。系统启动后，当我们首次执行 `adb` 命令时，服务端进程将会自动启动。
...
"adbd"是运行在Android设备上的进程，用于连接ADB Server并运行收到的指令。

# 安装与配置
ADB包含在Android SDK中，如果我们已经在计算机上安装了Android SDK，可以在 `<Android SDK安装目录>/platform-tools/` 路径下找到ADB的可执行文件。为了方便地在命令行中全局调用ADB命令，我们可以将该目录添加到操作系统的 `PATH` 环境变量中。
...
如果我们并不需要完整的Android SDK，可以从以下页面下载单独的ADB组件包： [🔗 ADB - 官方下载页面](https://developer.android.google.cn/tools/releases/platform-tools#downloads) 。

# 基本应用
## 通过USB连接设备
我们首先需要进入 `系统设置 -> 关于手机` 页面，连续点击5次“版本号”，启用隐藏菜单“开发者选项”；然后进入 `系统设置 -> 系统 -> 开发者选项` 页面，启用“USB调试”选项；并通过USB线缆将Android设备与计算机相连。

接下来，我们在计算机终端上输入 `adb device` 命令启动ADB服务器，此时Android设备就会弹出ADB授权窗口，当我们点击“允许”按钮后，设备就会连接到ADB服务器。
...
- 对于手机、平板电脑等便携设备，使用有线调试并不方便。
- 部分设备的USB接口开启ADB后无法识别U盘等设备，此时我们无法调试相关功能。

为了解决上述问题，我们可以使用设备的无线网卡与计算机互联。

我们首先需要确保计算机和Android设备之间IP可达，然后使用USB线缆连接计算机并完成ADB授权，并在计算机终端输入 `adb tcpip <端口>` 命令设置ADB的TCP/IP端口：
...
接着执行 `adb connect <设备IP>:<端口>` 命令，使计算机与Android设备互联。
...
## 选择设备
当有多个设备连接至计算机时，直接输入ADB命令将会出现错误："more than one device and emulator"，这是因为ADB不知道应当将命令发送给哪个设备。

我们可以在ADB命令的第一个参数位置添加 `-s <设备标识符>` 选项，指定当前操作的设备。
...
## ADB Server
ADB Server进程通常是自动管理的，我们无需人工干预；若服务端功能异常，我们可以尝试通过以下命令重启ADB Server。
...
如果ADB Server默认的5037端口被占用导致启动失败，我们可以使用 `-p <端口号>` 选项指定一个其他端口。
...
## ADB Shell
我们可以在计算机的终端中执行 `adb shell` 命令进入Android设备的Shell，并进行其他交互式操作，此时命令提示符将会发生改变；当我们想要返回计算机的终端时，需要按下 `Ctrl+D` 组合键或执行 `exit` 命令。
...
有时我们希望使用ADB Shell执行命令语句，但不进入ADB Shell命令行，此时可以使用 `adb shell "<命令语句...>"` 命令。这是ADB Shell的非交互式使用方法，我们可以在编写脚本文件等场景中使用。
...
计算机的终端显示了Android设备 `free -h` 的执行结果，但并未进入ADB Shell，命令提示符仍然为计算机的终端。
...
如果文件传输失败，通常是因为ADB Shell的默认用户对目标路径无写入权限；对于拥有Root权限的设备，我们可以先执行 `adb root; adb remount;` 命令提升权限后再尝试传输。

PC端的文件路径中允许出现通配符，此时可以仅传输符合规则的部分文件。
...
当我们使用通配符时，PC端路径不能被双引号包围，这意味着通配符无法与带有空格的路径并存。

### 向PC端传输文件
我们可以使用 `adb pull <Android端目标路径> <PC端文件或目录路径>` 命令将Android设备中的文件传输至PC。

```text
# 将Android设备中的"test.txt"文件复制到PC当前目录
...
`adb pull` 命令可以传输单个文件或多级目录，但无法使用通配符。

## 截屏或录屏
我们可以使用ADB截取设备的屏幕内容并保存至文件。
