...
[🔗 Android ABI支持信息 ](https://developer.android.google.cn/ndk/guides/abis)
...
我们可以将APK文件解压缩后，检查其中的动态链接库来确定应用程序的位数；也可以将应用程序安装到设备上，然后通过 [🔗 LibChecker ](https://github.com/LibChecker/LibChecker) 工具进行判断。
...
`/data/app/<应用包名>/lib/arm64` `/data/app/<应用包名>/lib/x86_64`
1、app 目录下的 so 库路径（）
2、系统 so 库存储路径（ 、、/vendor/lib64）
