...
UserID:唯一标识每个用户，0是第一个用户，其他用户ID默认以10为倍数递增，例如：10、20、30等。

AppID:采用沙盒机制，因此每个应用有独立的ID作为表示，在每个应用被安装时分配并持久化 uid记录在data/system/packages.xm ，当应用被卸载重装后才会改变。范围从10001开始递增。

UID：UserID和AppID的组合，用于唯一标识应用进程，公式如下：

UID = (UserID * 100000) + (AppID % 100000)

如果某个应用程序的AppID为10105，则User 0 的UID为 `10105 = 0 + 10105`、User 10的UID为 `1010105 = 1000000 + 10105`

ps命令可以看到进程的UID信息

```text
emulator64_x86_64:/ # ps -A | grep launcher
...
如果我们只知道UID，也可以反推出UserID与AppID，三者之间的换算方法见下文示例代码：
