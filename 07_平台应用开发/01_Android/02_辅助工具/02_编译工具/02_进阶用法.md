...
- 在Android Library中，我们不能使用 `from(components.getByName("java"))` 语句发布程序包，此处将其替换为 `afterEvaluate { artifact(tasks.getByName("bundleReleaseAar")) }` 语句，等待"bundleReleaseAar"任务执行完毕，并将产物AAR作为发布组件。
- 发布产物的类型从JAR变更为AAR后，我们需要同步更改 `pom {}` 小节中的 `packaging` 属性。
- 在Gradle 8.0及更高版本中，Maven Publish插件不会自动将Android Library的依赖组件信息写入POM文件，我们需要新增一些逻辑代码，遍历Gradle的依赖组件，将其转为XML格式并写入POM文件。

## 发布源码包
我们可以在Android Library的Gradle配置文件中添加以下配置项，以实现源码包的发布。
