...
该配置文件中的 `pluginManagement {}` 与 `dependencyResolutionManagement {}` 小节用于配置Gradle插件与依赖组件仓库信息，详见相关章节： [🧭 Gradle - 仓库管理](../../../../04_软件应用技巧/07_编程工具/02_编译构建/02_Gradle/03_依赖管理.md#仓库配置) 。
...
Android Gradle插件(Android Gradle Plugin, AGP)使Gradle具有生成Android产物的能力，功能包括编译Java源码、打包资源文件、配置版本信息等。
...
上述配置文件中出现的Gradle插件详细信息可参考下文内容：

🔷 `com.android.application`

该AGP的作用是生成Android应用程序，我们将应用该插件的模块简称为“APP模块”，它们的构建产物为APK文件，可以被安装到设备中并运行。

🔷 `com.android.library`

该AGP的作用是生成Android库组件，我们将应用该插件的模块简称为“LIB模块”，它们的构建产物为AAR文件，只能作为APP模块或其他LIB模块的依赖项，提供可复用的能力，无法直接被安装到设备中。

🔷 `org.jetbrains.kotlin.android`

该插件不属于AGP，它由Jetbrains提供，能够使Android模块支持Kotlin语言。

对于只使用Java语言的模块，我们不必使用该插件。
...
配置文件编写完毕后，我们在Android Studio中执行一次Gradle Sync操作，即可在Build Variant面板中查看新增的自定义类型。
...
# 签名配置
我们可以在APP模块中创建多个签名配置，Gradle编译产生APK后，将会根据编译类型自动选择对应的签名配置为APK签名，简化软件发布的流程。
...
|       序号        |                             摘要                             |
| :---------------: | :----------------------------------------------------------: |
| [案例一](#案例一) |     多个LIB模块的BuildConfig冲突，导致App模块编译失败。      |
| [案例二](#案例二) | LIB模块使用Java 11，APP模块使用Java 8，导致App模块编译失败。 |

</div>

## 案例一
### 问题描述
Application模块编译失败，错误信息为： `Type <包名>.BuildConfig is defined multiple times` 。

```text
> Task :app:mergeLibDexDebug FAILED
ERROR: RootProject/app/build/.transforms/655facb923193c287625cad02831b812/transformed/debug/<包名>/BuildConfig.dex: D8: Type <包名>.BuildConfig is defined multiple times: ...
...
### 问题分析
我们首先查看错误消息中提及的网页： [🔗 Java 8 Support](https://developer.android.com/studio/write/java8-support.html) ，可以得知问题原因如下：

APP模块使用Java 8编译，LIB模块使用Java 11编译，Java 11新增的特性无法在Java 8中使用，因此导致了编译失败。

### 解决方案
Android官方提供了名为“脱糖(Desugaring)”的兼容方案，我们可以在APP模块中开启该特性，使Gradle向产物中添加Java 8相比Java 11缺少的实现代码，确保LIB模块中的功能可以正常运行。
...
|       序号        |                         摘要                          |
| :---------------: | :---------------------------------------------------: |
| [案例一](#案例一) | 编译失败，错误消息为“缺少某个模块的 `*-r.txt` 文件”。 |

</div>

## 案例一
### 问题描述
Gradle编译包含AAR依赖项的模块时，出现以下错误：
...
### 问题分析
该问题的原因是我们删除了Gradle缓存目录 `<用户目录>/.gradle/caches/` 中的AAR缓存，Gradle不会感知到这种变化，仍然认为AAR已经下载完毕，所以会出现此错误。

### 解决方案
我们需要终止所有正在运行的Gradle进程，并删除Gradle进程的缓存文件，使缓存状态一致。例如：当前工程使用的Gradle版本为"8.5"，我们应当删除 `<用户目录>/.gradle/caches/8.5/` 目录并尝试再次编译。
