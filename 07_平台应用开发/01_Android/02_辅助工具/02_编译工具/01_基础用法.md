# 目录结构
目前Gradle是Android官方指定的APP构建工具，当我们使用Android Studio创建一个新的工程后，将会生成名为"app"的子模块，以及一些Gradle配置文件，它们的目录结构如下：
...
|       序号        |                   摘要                    |
| :---------------: | :---------------------------------------: |
| [案例一](#案例一) | 多个模块的BuildConfig冲突，导致编译失败。 |

</div>

## 案例一
### 问题描述
应用程序模块编译失败，错误信息为： `Type <包名>.BuildConfig is defined multiple times` 。

```text
> Task :app:mergeLibDexDebug FAILED
ERROR: Project/app/build/.transforms/655facb923193c287625cad02831b812/transformed/debug/<包名>/BuildConfig.dex: D8: Type <包名>.BuildConfig is defined multiple times: ...
com.android.builder.dexing.DexArchiveMergerException: Error while merging dex archives:
Learn how to resolve the issue at https://developer.android.com/studio/build/dependencies#duplicate_classes.
```

### 问题分析
当前模块依赖多个子模块，其中两个子模块的命名空间配置是相同的，命名空间决定了BuildConfig、R等自动生成代码的存放路径，因此当它们被同一模块依赖时，各自的BuildConfig无法合并，就会产生冲突导致编译失败。

### 解决方案
每个模块必须拥有全局唯一的命名空间属性，不能与其他模块冲突。
