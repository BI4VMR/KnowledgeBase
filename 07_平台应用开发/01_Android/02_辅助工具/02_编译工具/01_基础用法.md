...
# Signing Config
Signing Config用于描述签名信息，我们可以在Application模块中创建多个签名配置，Gradle编译产生APK后，将会根据编译参数自动使用对应的配置为APK签名，简化软件发布的流程。

每个Application模块都有一个名为"debug"的默认签名配置，其使用的签名文件为 `<用户主目录>/.android/debug.keystore` ，我们可以在模块 `build.gradle` 文件的 `signingConfigs {}` 小节中覆盖默认配置，也可以创建一些新的签名配置。
