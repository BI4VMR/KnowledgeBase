...
# 启动链路
我们的应用程序如果拥有公开的Activity，它们就可以被多种外部组件启动。

我们有时需要获取启动当前Activity的组件信息，用于显示返回按钮帮助用户返回原应用程序；或者分析启动链路，此时我们可以使用Activity的 `URI getReferrer()` 方法，它的返回值表明了启动者的身份。
...
- 当程序A通过 `startActivity()` 方法启动程序B的Activity时，返回值为程序A的URL，格式为： `android-app://<应用A的包名>` 。
