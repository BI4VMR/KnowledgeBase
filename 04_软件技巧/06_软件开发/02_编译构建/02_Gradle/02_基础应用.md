# 初始化工程
若要创建一个新的 Gradle 工程，我们首先应当执行 `gradle init` 命令进行初始化。
...
# Gradle Wrapper
## 简介
Gradle版本更新较为频繁，新旧版本可能无法相互识别部分配置语法，因此全局安装的版本难以适用于多个工程。

Gradle Wrapper是工程根目录下的脚本文件，可以调用指定版本的Gradle程序进行编译，通过该脚本我们可以为每个工程配置不同的Gradle版本，解决不同工程共存时的兼容性问题。
...
## 离线安装
每当我们使用工程中的Gradle Wrapper脚本执行任务时，若指定的Gradle版本未被安装，脚本就会自动下载当前版本。如果此时终端所处的网络环境不佳，下载过程可能会中断，导致无法进行后续的工作。在这种情况下，我们可以手动下载Gradle版本，或者从其他设备复制已有的Gradle版本，跳过自动下载过程。
