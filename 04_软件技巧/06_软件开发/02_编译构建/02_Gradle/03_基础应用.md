# 初始化工程
对于一个新创建的空工程，我们首先需要执行 `gradle init` 命令进行初始化。Gradle会通过交互式向导询问若干问题，并根据用户的应答生成基本配置文件。
...
> 🚩 提示
>
> 如果我们通过IntelliJ IDEA或Android Studio等IDE创建Gradle工程，以上文件将会自动生成，无需再手动执行 `gradle init` 命令。
