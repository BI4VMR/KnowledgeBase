...
| [案例三](#案例三) |      构建工程时出现错误： `Could not parse module metadata` 。      |
| [案例四](#案例四) |     构建脚本引用Java API时出现错误： `Unresolved reference` 。     |
...
## 案例二
### 问题描述
构建工程时出现错误，错误信息为： `Could not parse module metadata` :

```text
Could not parse module metadata http://192.168.1.1:8081/repository/maven-private/net/example/baselib/1.0.0/baselib-1.0.0.module
...
匿名权限开启，已登录管理员的浏览器可以正常下载文件，但使用未登录的浏览器访问链接，返回JFrog页面，无内容。

尝试更改匿名用户权限，无效，可能是旧版本的错误，删除所有文件后升级到最新版本，问题解决。

nexus-3.79.1-04-linux-x86_64.tar
