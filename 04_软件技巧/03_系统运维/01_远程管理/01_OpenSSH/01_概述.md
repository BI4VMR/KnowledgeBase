...
| [案例四](#案例四) | 连接服务器时出现警告： `not using a post-quantum key exchange algorithm` 。 |
...
### 问题分析
在OpenSSH v10.1及更高版本中，客户端推荐使用能够对抗量子计算机的新式密钥交换算法，如果服务器不支持此类算法，则会显示警告信息。

### 解决方案
如果我们能够管理目标服务器，为了提高安全性，最好升级服务器的SSH软件版本，使其支持较新的密钥交换算法。

如果我们无法升级目标服务器，可以在客户端添加配置，不再使用新式密钥交换算法。

🔷 方法一

在SSH命令行中添加 `-oKexAlgorithms=-sntrup761x25519-sha512@openssh.com` 选项，该方法适用于临时性的调试工作。
