...
安装pcp后仍提示该错误，安装 dnf install python3-pcp 后问题解决。


# Tomcat

Tomcat 10 部署Web应用后出现404

应用在9版本上可以正常使用

Tomcat10将API包名
从原先的 javax.servlet 改成了 jakarta.servlet
...
如果没有源码则只能将应用部署到版本9实例上，不能使用版本10。
...
GRUB_CMDLINE_LINUX_DEFAULT="quiet splash ttm.pages_limit=27000000 ttm.page_pool_size=27000000 amdttm.pages_limit=27000000 amdttm.page_pool_size=27000000 apparmor=0  amd_iommu=off"
...
Linux内存分配的最小计量单位是 4KB，所以 (262144 KB = 4KB * 256 * 1024) 是 1G ，根据实际情况计算出合适的数值，例如 16G * 262144 = ...
目前版本超过 108GB 左右会不稳定 最好设置小于 28311552 的数值。
...
exiftool <文件名>
exiftool "D:\Download\20250124_141240_DSC0080.NEF"
...
修改文件后如果未指定输出目录，默认会将源文件重命名为 原名_original ，然后生成文件

-overwrite_original 原地修改

-overwrite_original_in_place 原地修改，继承文件创建与修改时间。
