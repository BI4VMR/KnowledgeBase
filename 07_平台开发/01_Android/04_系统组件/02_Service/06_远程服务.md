...
如果我们需要传递自定义类型的数据，必须为其实现Parcelable接口，以便进行序列化传输。关于Parcelable接口的知识详见相关章节： [🧭 Parcelable接口](../01_通用组件/05_Parcelable接口.md) 。
...
AIDL文件的语法与Java接口类似，包声明语句中的路径为该文件在 `<模块根目录>/src/main/aidl/` 目录内的路径。
