...
此处的 `import` 语句应当指向 `DownloadItem.aidl` 所在包名，而非 `DownloadItem.java` 在Java源码集中的包名。

第三步，我们在服务端与客户端分别执行一次Make或Build任务，然后修改接口实现，适配新的数据类型。

# 数据方向标签
## 简介
我们在定义AIDL接口时，部分参数前可以指定一个方向标签，例如前文“示例二”中的 `addTask(in DownloadItem task)` 接口。该标签可以取值为 `in` 、 `out` 或 `inout` ，能够控制两个进程之间参数的同步情况。
