...
每当外部组件请求启动本服务时，回调方法 `onStartCommand()` 将会触发，我们在此处创建子线程，并使其休眠5秒，模拟下载的过程。
...
在上述代码中，当Service被启动时，我们立刻调用 `startForeground()` 方法绑定通知，Service即成为前台服务，通知栏应当出现通知

`createNotification()` 方法用于创建通知，此处忽略具体实现，详情可参考示例代码。
通知的相关知识可参考以下章节： [🧭 通知管理](../05_核心能力/02_通知管理.md) 。



当服务被终止时，我们调用了 `stopForeground()` 方法移除通知。实际上这没有什么
