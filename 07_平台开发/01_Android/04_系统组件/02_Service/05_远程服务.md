# 简介
在操作系统中，一个进程无法直接读取另一个进程的数据或调用其中的接口，为了实现进程间通信(IPC)，我们需要定义客户端与服务端均认可的编程接口，实现双方的数据交互。

Android平台提供了AIDL(Android Interface Definition Language)规范，用于描述进程间通信所使用的公共接口。AIDL文件会被Gradle编译为Java代码，基于Android平台的Binder驱动实现IPC，我们可以通过绑定服务使用这些接口。
...
在本示例中，我们通过AIDL实现前文示例中的下载服务，由客户端提供用户界面，服务端提供业务逻辑，实现基本的跨进程通信。

第一步，我们编写AIDL接口，定义服务端所支持的能力。

AIDL文件的后缀为 `.aidl` ，应当放置于 `<模块根目录>/src/main/aidl/<包名>/` 目录中，此处的 `<包名>` 决定了AIDL文件编译生成的Java文件所在目录。

我们首先创建一个新的工程 `S04_AIDL-Server` 作为服务端，并创建AIDL文件 `src/main/aidl/net/bi4vmr/aidl/IDownloadService.aidl` ，编写以下内容：
...
AIDL文件的语法与Java接口类似， `package` 语句中的路径应当与该文件在 `<模块根目录>/src/main/aidl/` 目录内的路径保持一致。此处我们在接口中定义了三个方法，表示客户端可以调用这些能力，而服务端必须实现对应的接口。

AIDL接口中的引用类型数据默认均不可为空，如果我们需要可空的参数或返回值，则需添加 `@nullable` 注解，例如： `void addTask(in @nullable String url)` 。

第二步，我们在服务端实现AIDL接口，提供接口对应的能力。

我们首先为服务端工程执行一次Build任务，使Gradle生成AIDL对应的Java类。编译完成后，我们可以检查模块的 `build/generated/aidl_source_output_dir/debug/out` 目录中是否存在 `IDownloadService.java` 等文件。

IDownloadService类中有一个抽象类IDownloadService.Stub，它继承自Binder类，我们需要在服务端Service内实现IDownloadService.Stub中的抽象方法，编写具体的业务逻辑。
...
Service编写完成后，我们在Manifest文件中注册该组件，并设置一个Action标签，以便客户端绑定该服务。
...
在上述配置文件中，我们声明了 `android:process=":service"` 属性，因此DownloadService将会运行在名为 `<服务端包名>:service` 的独立进程中，即使我们从服务端的默认进程 `<服务端包名>` 绑定该服务，仍然属于跨进程通信，服务端示例工程中也有一份客户端的逻辑代码，以便我们验证这一行为。

第三步，我们从客户端绑定该服务，并调用AIDL接口。

客户端若要访问AIDL接口，也需要引用AIDL文件编译所生成的Java类。此处为了方便进行实验，我们直接将AIDL文件从服务端工程复制到客户端工程，并保持相同的目录结构，然后编译一次客户端工程生成Java类。

在实际应用中，我们通常会将AIDL文件作为一个独立的模块，由服务端依赖该模块并实现业务接口逻辑，由客户端依赖该模块并实现绑定、解绑等状态管理，然后将客户端作为JAR包或AAR包发布到Maven仓库；调用者无需关心客户端的状态管理等细节，只需集成客户端SDK即可远程调用服务端提供的能力。
...
在 `onServiceConnected()` 回调方法中，我们可以通过 `IDownloadService.Stub.asInterface()` 方法将第二参数 `service` 转换为IDownloadService类型，方便后续调用AIDL业务接口。

当服务连接成功后，我们便可以通过IDownloadService类型的全局变量 `downloadService` 远程调用服务端业务接口，此处以 `addTask()` 接口为例：
...
在执行远程调用之前，我们应当主动判断服务的连接状态，如果服务端进程终止但尚未来得及通知客户端，客户端的Binder引用不会被置空，此时远程调用会导致DeadObjectException异常。

在远程调用过程中，除了业务自身的异常，还有可能发生RemoteException异常，例如：列表元素过多超出AIDL限制、权限不足等，我们应当添加异常捕获语句。


# 服务状态检测
TODO !!!!!!!!!!!
前文章节中我们在客户端通过ServiceConnection回调接口监听服务连接和断开事件，并同步更新Binder变量通告该状态，这在绝大多数场景中是可以正常工作的，但某些场景存在问题。

ServiceConnection只有发起bind的调用者持有，在普通应用中发起者和binder持有者都是同一个类，例如TestUIBase，但系统应用可能采用分离设计，binder持有者与bind发起者不再同一处，无法通过ServiceConnection监听服务状态。

当服务端Crash或系统资源不足被杀死后，ServiceConnection由AMS回调，客户端可能不会立即知晓，如果此时客户端使用已经失效的Binder发起远程调用，就会导致DeadObjectException异常。


ADB# ps -ef | grep net.bi4vmr.study.system.service.aidlserver

u11_a116     23232   750 1 07:47:12 ?     00:00:00 net.bi4vmr.study.system.service.aidlserver:service


ADB# kill -9 23232

07:47:36.847 23155-23155 TestApp-Client-TestUIBase      net...ice.aidlserver  I  ----- 添加任务 -----
07:47:36.847 23155-23155 System.err                     net...ice.aidlserver  W  android.os.DeadObjectException
07:47:36.847 23155-23155 System.err                     net...ice.aidlserver  W  	at android.os.BinderProxy.transactNative(Native Method)
07:47:36.847 23155-23155 System.err                     net...ice.aidlserver  W  	at android.os.BinderProxy.transact(BinderProxy.java:584)
07:47:36.847 23155-23155 System.err                     net...ice.aidlserver  W  	at net.bi4vmr.aidl.IDownloadService$Stub$Proxy.addTask(IDownloadService.java:147)
07:47:36.847 23155-23155 System.err                     net...ice.aidlserver  W  	at net.bi4vmr.study.base.TestUIBase.testAddTask(TestUIBase.java:125)


07:47:37.627 23155-23155 TestApp-Client-TestUIBase      net...ice.aidlserver  I  连接已就绪。
