...
解析所有Activity等组件会导致查询时间过长，因此 `getPackageInfo()` 方法默认只返回基本信息，额外信息对应的属性均为空值；若要解析额外信息，我们可以查看源码中的注释确定它们的Flag值，并在 `getPackageInfo()` 方法的第二参数中指明。

自从Android 13 (API 33)开始，我们应当使用PackageInfoFlags表示标志位，它的静态方法 `of(int value)` 可以将数值形式的标志位组合转为PackageInfoFlags对象。为了提高兼容性，后文示例默认都使用数值形式的Flag组合。

若目标软件包未被安装在设备上， `getPackageInfo()` 以及类似的方法都会抛出 `PackageManager.NameNotFoundException` 异常，如果我们只想判断某个应用程序是否已被安装，也可以调用 `getPackageInfo()` 方法并检查是否出现异常，后文示例将会省略对此异常的处理语句。

## 应用信息
ApplicationInfo表示应用程序Manifest文件 `<application>` 标签的属性，以及其他应用级别的信息，常用属性与方法详见下文列表：

- `packageName` : 包名。
- `loadLabel(PackageManager pm)` : 应用名称。
- `loadIcon(PackageManager pm)` : 带有徽章的图标。
...
## 组件信息
PackageManager提供了以下方法便于我们获取软件包中的Activity、Service等组件信息：
