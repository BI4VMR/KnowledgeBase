...
如果我们并不需要完整的Android SDK，也可以在 [🔗 ADB - 官方下载页面](https://developer.android.google.cn/tools/releases/platform-tools#downloads) 下载独立的ADB软件包。
...
## 选择设备
当多个设备同时连接至PC时，直接输入ADB命令将会出现错误： `more than one device and emulator` ，这是因为ADB不知道应当将命令发送给哪个设备。
...
如果我们调用 `screencap` 命令时未指定目标文件，图像数据将通过命令行标准输出直接传递至PC端，这在自动化测试场景中非常有用。此时我们应当使用 `adb exec-out` 命令调用 `screencap` 命令，因为 `adb shell` 命令将根据PC端系统类型选择换行符，导致不同平台的输出数据格式不一致，而 `adb exec-out` 命令在任何系统中都会保持原样输出，不会修改换行符。
