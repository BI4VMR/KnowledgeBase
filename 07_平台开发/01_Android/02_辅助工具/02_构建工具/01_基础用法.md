...
# SourceSets
`sourceSets {}` 小节用于控制源码及资源目录的结构，如果工程规模较大，将所有源码或资源都放置在默认的 `src/main` 目录中将导致难以维护，此时我们可以新建一些目录放置源码或资源，并通过SourceSets配置进行注册。
...
当我们添加上述配置后，可以在 `src/main` 中新建 `java-vendor` 和 `kotlin` 目录放置源代码，并新建 `res-extra` 目录放置资源文件。
