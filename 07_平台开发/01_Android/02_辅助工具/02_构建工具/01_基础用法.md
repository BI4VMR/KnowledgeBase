# 目录结构
Android官方指定的应用程序构建工具是Gradle，当我们使用Android Studio创建一个新的工程后，将会生成名为 `app` 的子模块，以及一些Gradle配置文件，它们的目录结构如下文代码块所示：
...
工程根目录下的 `settings.gradle` 配置文件包含工程全局属性、子模块声明、仓库声明等信息。
...
上述配置文件中的 `pluginManagement {}` 与 `dependencyResolutionManagement {}` 小节用于配置Gradle插件与依赖组件仓库信息，详见相关章节： [🧭 Gradle - 仓库管理](../../../../04_软件技巧/06_软件开发/02_编译构建/02_Gradle/04_依赖管理.md#仓库配置) 。

该配置文件中的剩余部分用于配置项目结构，声明各个子模块，详见相关章节： [🧭 Gradle - 模块管理](../../../../04_软件技巧/06_软件开发/02_编译构建/02_Gradle/03_基础应用.md#模块管理) 。
...
Android相关的插件都不是Gradle官方插件，因此我们在声明它们时必须指明版本号。由于根工程无需放置Android源代码，此处添加了 `apply false` 属性，使插件不对根工程生效。

Android Gradle插件(Android Gradle Plugin, AGP)使Gradle具有生成Android产物的能力，其功能包括编译Java源码、打包资源文件、配置版本信息等。

AGP的版本需要与Gradle版本相匹配，相关信息可在Android Developer网站的 [🔗 AGP版本说明](https://developer.android.com/build/releases/about-agp?hl=zh-cn#updating-gradle) 页面中查询；所有已发布的版本号可参考 [🔗 Google Maven - AGP](https://maven.google.com/web/index.html?#com.android.application:com.android.application.gradle.plugin) 页面。
...
该配置文件中的 `dependencies {}` 小节用于声明当前模块所依赖的外部组件，详见相关章节： [🧭 Gradle - 依赖管理](../../../../04_软件技巧/06_软件开发/02_编译构建/02_Gradle/04_依赖管理.md) 。


# DefaultConfig
`defaultConfig {}` 小节用于配置模块的基本信息，例如：包名、版本号等，常用的属性如下文列表所示：

- `applicationId` : 包名。
- `versionCode` : 数字版本号。从 `1` 开始递增的整数，系统通过该值比较软件包的新旧，每次发布新版本时我们应当将该值加一。
- `versionName` : 文本版本号。供人类阅读的版本信息，我们可以添加重要的标记，例如： `1.2.5_20260101_debug` 分别对应 `<版本>_<日期>_<类型>` 。
- `minSdk` : 最低API版本，默认值为 `1` 。目标设备的系统版本必须大于或等于该值，否则无法安装。
- `targetSdk` : 目标API版本，默认值跟随 `minSdk` 。该属性指明应用程序期望的系统版本，运行时系统将会启用或禁用某些特性，例如：API < 30时程序可以不适配“分区存储”特性，但API >= 30时必须进行适配。
- `compileSdk` : 编译API版本。该属性决定当前模块编译时所使用的Android SDK版本，不影响运行时的行为。较新的SDK包含接口弃用警告与新的语法检查规则，我们应当尽快适配这些变更，因此该属性应当跟随最新发布的SDK版本。这是一个全局属性，需要在 `defaultConfig {}` 小节外部声明。
...
每个模块初始时拥有 `debug` 和 `release` 两个类型，其中 `debug` 类型面向开发者，默认允许进行调试、禁用代码混淆； `release` 类型面向最终用户，默认不可调试、启用代码混淆。
...
# SourceSets
`sourceSets {}` 小节用于控制源码及资源目录的结构，部分工程规模较大，若将所有源码或资源都放置在默认的 `src/main` 目录中，维护难度较高；我们可以新建目录放置源码或资源，并在SourceSets配置中注册，使这些目录参与编译。
...
在上述示例中， `srcDir()` 方法表示添加单个路径， `srcDirs()` 方法表示添加多个路径。

当我们在工程中新增上述配置之后，可以在 `src/main` 目录下新建 `java-vendor` 、 `kotlin` 等目录放置源代码，并新建 `res-extra` 目录放置资源文件。


# Kotlin配置
## 添加Kotlin支持
如果一个工程原本并未配置任何Kotlin相关组件，我们可以在Gradle配置文件中新增以下内容，使其支持Kotlin语言。
...
`org.jetbrains.kotlin.android` 插件的版本号与Kotlin版本保持同步，所有已发布的版本号可参考 [🔗 Gradle Plugins - Kotlin Android](https://plugins.gradle.org/plugin/org.jetbrains.kotlin.android) 页面。
...
该配置对于kotlin是默认值，可以用不加，但如果kotlin目录下混合放置java代码，必须配置，否则javac编译器不会识别它们。
...
| [案例四](#案例四) | 构建工程时出现错误： `D8: java.lang.NullPointerException` 。 |
| [案例五](#案例五) |  Gradle Sync出现错误： `java.lang.NullPointerException` 。   |
...
### 问题分析
APP模块引用了JAR包，该JAR包也是由我们自行上传到私有Maven仓库服务器的，问题出现的原因不明，但执行后文操作可以解决。

### 解决方案
我们使用管理员账号登录到Maven仓库服务器，删除现有的JAR包并尝试重建索引，然后重新上传JAR包。

<div align="center">

![重建索引](./Assets_基础用法/疑难解答_案例四_重建索引.jpg)
