...
# Packaging Options
`packagingOptions {}` 小节用于控制生成APK时打包Java资源（不包括Android资源）与动态链接库文件的规则，典型用法如下文代码块所示：
...
- `jniLibs.excludes` : 排除符合规则的动态链接库文件。
- `jniLibs.pickFirsts` : 只打包首个符合规则的动态链接库文件，后续同类文件将被忽略。
- `jniLibs.doNotStrip` : 打包时不对动态链接库文件进行裁剪优化。
- `resources.excludes` : 排除符合规则的资源文件。
- `resources.pickFirsts` : 只打包首个符合规则的资源文件，后续同类文件将被忽略。
- `resources.merge` : 若多个模块具有打包后路径相同的文件，将产生冲突并导致编译失败，此配置项表示将这些文件合并为一份，避免冲突。


从 AGP 7.0 开始， BuildConfig  类默认不再自动生成，需要显式在  buildFeatures  中开启  buildConfig = true  才会生成。
...
当我们选择某个Flavor进行编译时，其中的属性会覆盖 `defaultConfig {}` 小节中的同名属性。
