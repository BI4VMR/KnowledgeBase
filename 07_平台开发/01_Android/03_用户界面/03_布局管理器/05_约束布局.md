# 简介
约束布局(Constraint Layout)是Google于2016年推出的一种布局管理器，它的控件排布方式与相对布局类似，但具有更丰富的规则与辅助工具，能够显著减少布局嵌套层数，提高渲染性能，目前已经成为Android Studio新建布局文件时的默认根布局。
...
# 元素宽高约束
match_parent:占满父布局可用空间，此时可以不设置约束条件，不推荐。
wrap_content:包裹自身内容，必须设置约束条件，内容大于约束目标时自身将会继续扩展不受约束条件限制。
0dp:表示由约束条件确定。内容不足也会占满约束条件所制定的空间，内容大于约束目标时不会继续扩展。

`app:layout_constraintWidth_default="spread"` 控制 0dp时的行为，spread即占满约束条件所制定的空间，wrap为包裹内容，内容大于约束目标时不会继续扩展。

layout_constrainedWidth=true，如果是wrapcontent，添加该属性后不会超出约束条件，等价于 0dp+`app:layout_constraintWidth_default=wrap`
