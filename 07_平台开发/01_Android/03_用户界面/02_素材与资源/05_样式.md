...
values-night中创建同名主题，深色模式启用时自动加载此处的资源
...
如需切换主题，请执行以下操作：
    ...
    在 API 级别 31 及更高级别中，使用 UiModeManager#setApplicationNightMode 让系统知道应用运行的主题。这样，系统就可以在启动画面期间匹配主题。

    在 API 级别 30 及更低级别上，使用 AppCompatDelegate.setDefaultNightMode() 切换主题。
