...
该主题的名称为 `Theme.Custom` ，继承于Material库中的 `Theme.Material3.DayNight` 主题。我们可以根据需要编写 `<item/>` 小节覆盖父主题中的属性， `windowActionBar` 、 `windowNoTitle` 等属性用于设置窗口标题栏等模块的样式； `textViewStyle` 、 `checkboxStyle` 等属性用于设置控件的样式。
...
"themes_internal_attrs.xml":

```xml
<!-- 自定义主题 -->
<style name="Theme.Custom" parent="Theme.MaterialComponents.DayNight">
    <!-- 覆盖全局属性 -->
    <item name="windowActionBar">false</item>
    <item name="windowNoTitle">true</item>
