...
程序运行时，资源目录下的.png,xml等文件会根据其类型生成对应的Drawable实例，在某些情况下，我们也可以用代码动态创建Drawable。

## BitmapDrawable

本质上是内封位图对象Bitmap的Drawable，关于Bitmap的相关知识详见 ... 此处省略。
...
## LevelListDrawable
此类drawable可以通过数值形式的level进行切换，常用于wifi 移动网络信号强度指示等场合。

level不能是负数，如果在xml中填写负数会导致XmlPullParserException，如果在代码中动态设置会导致图像不显示。
如果在代码中设置xml范围未覆盖的level，图像也会消失,最好在设置之前通过代码判断输入值范围,避免超出范围。
