...
此处 Row 的主要轴（水平方向）被设为与容器末端对齐，因此子元素 C 的末端紧贴父容器末端；次要轴被设为垂直居中，因此所有元素的中点与父容器的垂直中心线对齐。

我们在子元素 B 的 Modifier 中配置了 `align(Alignment.Bottom)` 语句，这将覆盖全局的 `verticalAlignment` 属性，此时元素 B 不再对齐到垂直中心线，而是与父容器底部对齐。

---

如果 Column 或 Row 中的所有子元素都需要配置相同的间距，我们可以在父容器处统一配置，而不必在元素之间添加 Spacer 或使用 `padding` 属性实现留白。

🟢 示例十一：元素间距。

在本示例中，我们为 Row 中的元素统一设置 `10 dp` 的间距。
...
`spacedBy(space: Dp, alignment: Alignment.Horizontal)` 方法可为 Row 的所有元素添加 `space` 指定的间距，与此同时也可配置对齐方式，此处我们将所有元素水平居中放置。

此时运行示例程序，并查看界面外观：

<div align="center">

![元素间距](./Assets_布局/Column和Row_元素间距.jpg)

</div>

---

除了线性排列子元素之外， Column 和 Row 还提供了一些使子元素在父容器可用空间内平均分布的方式：

- `SpaceBetween` : 首尾两个元素紧贴主要轴的两端，其余元素在主要轴上平均分布。
- `SpaceEvenly` : 所有元素在主要轴上平均分布，首尾元素与容器边界的距离等于元素之间的距离。
- `SpaceAround` : 所有元素在主要轴上平均分布，首尾元素与容器边界的距离等于元素之间距离的一半。

<br />

🔵 示例十二：元素平均分布。

在本示例中，我们以 Row 为例，将其 `horizontalArrangement` 属性分别设为不同的平均分布方式，观察显示效果。

<div align="center">
    <img src="./Assets_布局/Column和Row_SpaceBetween.jpg" width = "300" height = "165" alt="SpaceBetween" align=center />
    &nbsp;&nbsp;
    <img src="./Assets_布局/Column和Row_SpaceEvenly.jpg" width = "300" height = "165" alt="SpaceEvenly" align=center />
    &nbsp;&nbsp;
    <img src="./Assets_布局/Column和Row_SpaceAround.jpg" width = "300" height = "165" alt="SpaceAround" align=center />
</div>

## 比例布局
我们可以在 Column 或 Row 子元素的 Modifier 中添加 `weight(weight: Float)` 属性，控制各个元素在父容器中占据空间的比例。

🟣 示例十三：按比例分配空间。

在本示例中，我们将 Row 的空间按比例分配给子元素。
...
我们令文本框 A 占据 Row 的 `70 %` 宽度；令文本框 B 占据 `30 %` 宽度。

此时运行示例程序，并查看界面外观：

<div align="center">

![按比例分配空间](./Assets_布局/Column和Row_按比例分配空间.jpg)

</div>

---

`fillMaxSize()` 与 `weight()` 都可以实现比例布局，但它们的工作方式有所不同。 `fillMaxSize()` 以容器尺寸为基准，乘以开发者设置的比例作为子元素尺寸。 Column 或 Row 首先在主轴上排列没有 `weight()` 属性的子元素，然后以剩余空间为基准，为所有设置了 `weight()` 属性的子元素设置尺寸。

🟤 示例十四：填充剩余空间。

假设某个页面具有标题栏和内容区域，其中内容区域需要根据屏幕高度伸缩，我们可以通过 `weight()` 属性实现。
...
在上述代码中，标题文本框未设置 `weight()` 属性，按照默认约束其高度与文本内容相同。内容文本框是容器中唯一具有 `weight(1F)` 属性的元素，因此高度与容器剩余空间相等，效果为占据所有剩余空间。

此时运行示例程序，并查看界面外观：

<div align="center">

![填充剩余空间](./Assets_布局/Column和Row_填充剩余空间.jpg)
...
# Box
## 简介
Box 将所有子元素重叠放置，较晚声明的元素覆盖在较早声明的元素之上，与 View 中的 FrameLayout 类似。
