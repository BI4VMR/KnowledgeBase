...
本示例中的两个构造方法是必选的，它们分别覆盖了动态创建View与通过XML声明View的场景。另外两个构造方法不会被外部调用，它们的作用是通过主题与样式配置控件，后文将详细地介绍它们。
...
`attrs` 、 `defStyleAttr` 和 `defStyleRes` 参数都用于初始化控件属性， `obtainStyledAttributes()` 方法将会自动处理它们，因此我们可以使参数较少的构造方法调用参数较多的构造方法，并将多余的参数设为默认值，初始化逻辑只需在参数最多的构造方法中统一编写即可。
