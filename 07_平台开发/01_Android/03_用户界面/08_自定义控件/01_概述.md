...
[🔗 示例工程：概述](https://github.com/BI4VMR/Study-Android/tree/master/M03_UI/C08_CtrlCustom/S01_Universal)
...
2. 调用者在布局文件中通过 `style` 属性引用的值。这些属性由 `set` 参数传入，遵循局部优于全局的原则，如果控件具有某些属性，将会覆盖 `style` 中的值；如果控件没有某些属性，则使用 `style` 中的值。
3. 当前主题所指定的控件样式。第三参数 `defStyleAttr` 指明该控件应当加载的主题属性名称，Context实例将从当前主题读取该属性的值并加载样式。
4. 第四参数 `defStyleRes` 指定的默认样式。如果当前主题未配置该控件 `defStyleAttr` 属性的样式（属性值为 `0` ），则使用默认样式。
5. 控件属性相关变量的默认值。我们通常会在控件内部声明一些全局变量，用于保存属性的值；如果解析属性阶段未匹配任何有效值，这些变量会保持声明时的默认值。
...
第一步，为测试控件AttrTypes声明两个属性，并添加该控件样式对应的主题属性。
...
<resources>
    ...
    <!-- 定义属性：测试控件在主题中的样式 -->
    <attr name="attrInTheme" format="reference" />
</resources>
```
...
```xml
<resources>
    <!-- 主题样式：通过主题属性引用，并传递给View构造方法第三参数。 -->
...
该样式用于配置测试控件在当前主题的外观，如果应用具有多个主题，我们可以为每个主题都准备一份不同的样式。

第三步，在主题中配置第一步创建的属性，使其指向前一步创建的样式。
...
<resources>
    ...
    <style name="Theme.Default" parent="Theme.MaterialComponents.DayNight.DarkActionBar">
        ...
        <!-- 为主题属性赋值，引用控件样式。 -->
        <item name="attrInTheme">@style/AttrTestThemeStyle</item>
    </style>
</resources>
```

此时测试控件、当前主题、样式三者产生了关联。如果具有多个主题，我们需要在每个主题中配置测试控件的样式。

第四步，修改测试控件的构造方法，使其加载第一步声明的主题属性。
...
此时运行示例程序，界面仅能读取到 `initParam1` 属性的值，而 `initParam2` 属性为空值。这是因为 `obtainStyledAttributes()` 方法将主题属性作为整体解析，若主题属性存在，只会解析其中声明的属性，对于未声明的属性则直接返回空值，不会在第四参数中进一步解析。

为了解决上述问题，我们需要继续配置默认样式，并使主题样式继承默认样式，解决部分属性缺失的问题。
...
我们创建名为AttrTestDefaultStyle的样式，作为测试控件的默认样式，此处应当为控件的所有属性都赋予默认值。先前创建的主题样式应当继承新增的默认样式。

第六步，在测试控件的构造方法中，将默认样式作为第四参数的值。
...
此时控件在以下场景都能正确加载默认样式的属性：

1. 解决前文出现的问题，主题样式仅需配置其关心的属性，其他属性可从默认样式继承。
2. 当前主题未配置测试控件的样式，则直接加载默认样式。
