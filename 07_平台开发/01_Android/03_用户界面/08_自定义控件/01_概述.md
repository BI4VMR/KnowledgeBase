...
具有一个参数与两个参数的构造方法是必选的，它们覆盖了动态创建View与通过XML声明View的场景。另外两个构造方法的参数 `defStyleAttr` 和 `defStyleRes` 与主题和样式有关，此处可以将其忽略，后文示例将详细介绍它们。
...
如果我们使用Kotlin语言编写控件，还可以利用默认参数将构造方法合并到一处，进一步简化代码：
