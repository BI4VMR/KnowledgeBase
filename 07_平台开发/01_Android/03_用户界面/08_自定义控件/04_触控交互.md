...
- [🔗 示例工程：触控交互](https://github.com/BI4VMR/Study-Android/tree/master/M03_UI/C07_CtrlCustom/S04_Interaction)


# 交互事件
在Android系统中，MotionEvent类用于描述用户与屏幕交互所产生事件，例如手指解除屏幕、手指在屏幕移动、鼠标点击屏幕等。

我们可以通过MotionEvent实例的 `int getAction()` 方法获取事件类型，常见的事件类型如下文所示：
...
我们创建一个Activity，并在根布局中放置一个按钮，然后重写Activity、根布局、按钮的事件处理方法，添加日志输出语句，运行示例程序后点击按钮并观察控制台消息：
...
当我们的手指接触到屏幕时， `ACTION_DOWN` 事件首先从Framework传递到Activity，Activity的 `dispatchTouchEvent` 方法被调用，该方法将会调用ViewGroup的dispatchTouchEvent方法，等待根容器进一步处理。

ViewGroup的dispatchTouchEvent方法，首先调用其 OnInterceptTouchEvent 方法，检查是否需要拦截事件，默认策略为不拦截，因此继续调用点击位置的子元素View的dispatchTouchEvent方法，等待其进一步处理。


View的dispatchTouchEvent方法被调用时，执行其OnTouchEvent方法，OnTouchEvent方法回调监听器，并确认自身已处理点击，返回true标记事件被消费，处理流程结束，父级View不毕再响应该事件。
