...
DecorView 添加content，然后将DecorView addview TYPE_BASE_APPLICATION
...
`WindowManager.LayoutParams.TYPE_` 系列常量用于指明窗口类型，可以控制层级，总体上数值越大层级越高，其数值在各版本可能不同，且数值大小不一定控制层级，具体层级由Framework实现而定。


应用添加窗口时，部分窗口在屏幕上同时只能显示一个，部分窗口可以多个显示，部分窗口需要特定权限。

总体上分为以下类型：

- 应用窗口（ Type 范围： `[1, 99]` ）：应用界面元素，例如 Activity、视频播放Surface等
- 应用子窗口（ Type 范围： `[1000, 1999]` ）：应用界面的附属元素，例如上下文菜单等，它们必须依附于Activity等，当Activity隐藏时必须同时关闭
- 系统窗口（ Type 范围： `[2000, 2999]` ）：直接由 WindowManagerService 管理的窗口，例如：状态栏、导航栏、系统错误提示窗口等。如果应用程序需要显示此类窗口，需要被授予特定的权限。
...
|       常量        |       数值       |       说明       |
| :---------------: | :--------------: | :--------------: |
| `TYPE_WALLPAPER` | 2013 | 用于显示壁纸，位于窗口堆栈的底层。 |
| `TYPE_APPLICATION_MEDIA` | 1001 | 视频播放 Surface。 |
| `TYPE_BASE_APPLICATION` | 1 | Activity View的窗口层级。 |
| `TYPE_APPLICATION` | 2 | 应用窗口 dialog。 |
| `TYPE_APPLICATION_STARTING` | 3 | 应用启动窗口（Splash/Preview Window）在 Activity 还没冷启动绘制完成前，系统先展示的占位窗口。 |
| `TYPE_APPLICATION_PANEL` | 1000 | 上下文菜单等。 |
| `TYPE_SYSTEM_DIALOG` | - | 上下文菜单等。 |
...
`int getWindowLayerFromTypeLw()` 方法返回了每个type对应的层级，不同版本方法名称、层级可能变化



# flag
LayoutParams.flags属性用于控制窗口的行为
...
- 模糊区域可以做移动变换，但必须始终可见，放入列表 Pager 等控件后翻页时无法被裁切，只能在翻页过程中暂时取消模糊。
