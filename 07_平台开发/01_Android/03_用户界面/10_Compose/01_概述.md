...
在View体系中，我们需要监听外部事件（例如：回调方法），并在事件发生时控制关联的UI组件作出响应（例如：更新文本内容、控制图片旋转等）；而在Compose体系中，我们通过函数描述UI组件，将可变属性（例如：文本内容、文本颜色等）作为函数的参数，每当外部状态变化时，Compose运行环境将会自动调用受影响组件的声明函数，传入新的参数以更新界面。
...
第一步，在模块的Gradle配置文件中添加对Jetpack Compose组件的依赖声明。
...
第二步，在模块的Gradle配置文件中添加Jetpack Compose的相关配置。
...
我们首先需要在 `buildFeatures {}` 小节中开启Gradle对Jetpack Compose的编译功能，然后在 `composeOptions {}` 小节中指明注解处理器的版本号。注解处理器的版本应与当前模块的Kotlin版本相匹配，对应关系可在 [🔗 Compose注解处理器版本](https://developer.android.com/jetpack/androidx/releases/compose-kotlin#pre-release_kotlin_compatibility) 网页中查询。

第三步，创建一个Activity，声明一些Compose组件，并将它们作为Activity的显示内容。
...
在Activity的生命周期方法 `onCreate()` 中，我们使用 `setContent()` 方法设置界面内容；最外层的TestComposeTheme是主题、较内层的Scaffold是根布局、最内层的Greeting即自定义Compose组件。
