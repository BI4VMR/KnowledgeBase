...
SQLite 提供了完整的 ACID（原子性、一致性、隔离性、持久性）事务支持，可以将多个数据库操作合并为一个逻辑单元。在 Android 平台上，正确使用事务可以确保数据的一致性和完整性。本节将详细介绍在 Android 上如何使用 SQLite 事务，并解释每个方法的作用及其成功与失败的跳转逻辑。
事务的基本概念

事务是一组 SQL 操作，这些操作要么全部执行成功，要么全部不执行。事务的主要特性包括：
...
在 Android 中，可以使用 SQLiteDatabase 类来管理事务。以下是一个典型的事务写法示例：
...
try {
    ...
    database.update("table_name", newValues, "whereClause", whereArgs);
...
方法详解
    ...
    beginTransaction()
        作用：开始一个新的事务。
        调用时机：在需要将一组操作作为一个整体执行时调用此方法。
        注意事项：每次调用 beginTransaction() 后，都需要调用 endTransaction() 来终止事务。

    setTransactionSuccessful()
        作用：标记当前事务为成功的状态。
        调用时机：当事务中的所有操作都成功执行后，调用此方法表示事务可以提交。
        注意事项：如果不调用此方法，则默认认为事务失败，事务会在 endTransaction() 调用时回滚。

    endTransaction()
        作用：结束当前事务。如果之前调用了 setTransactionSuccessful()，则提交事务；否则，回滚事务。
        调用时机：在 try-catch-finally 结构中，finally 块内调用此方法以确保事务总是被正确地终止。
        注意事项：每次调用 beginTransaction() 都需要对应一次 endTransaction()。
...
成功与失败的跳转逻辑
    ...
    成功路径：
        ...
        执行一系列数据库操作。
        如果所有操作成功，调用 setTransactionSuccessful() 标记事务为成功。
        最后，调用 endTransaction() 提交事务。

    失败路径：
        调用 beginTransaction() 开启事务。
        执行一系列数据库操作。
        如果某一步操作抛出异常，捕获异常并记录日志。
        不调用 setTransactionSuccessful()，直接进入 finally 块。
        调用 endTransaction() 回滚事务。

事务的隔离级别

Android 中的 SQLite 事务是最高隔离级别（Serializable），这意味着在一个事务开启后，其他事务所在线程将被阻塞，直到首个事务执行完毕后才会继续运行。这种隔离级别确保了事务之间的互斥性，但也可能导致性能下降，尤其是在高并发场景下。
...
在 Kotlin 和协程环境中，事务的处理需要注意线程切换的问题。由于 Room 支持协程，可以在 DAO 方法上使用 suspend 关键字来简化异步操作。然而，事务的开启和关闭仍然需要手动管理。
        ...
        launch(Dispatchers.IO) {
            try {
                ...
                database.runInTransaction {
                    // 业务操作...
                    userDao.insert(User(id = 1, name = "John Doe", age = 30))
                    userDao.update(User(id = 1, name = "Jane Doe", age = 30))

                    // 这里不需要显式调用 setTransactionSuccessful()
                }
            } catch (e: Exception) {
                Log.e(TAG, "操作失败，事务回滚！", e)
            }
...
注意事项
    ...
    线程安全：Room 的 DAO 方法通常是线程安全的，但事务管理需要小心处理线程切换。
    runInTransaction：Room 提供了 runInTransaction 扩展函数，可以在协程中方便地管理事务。该函数会自动处理事务的开启和关闭，无需手动调用 beginTransaction 和 endTransaction。
    异常处理：在事务块内捕获异常并进行适当的处理，以确保事务能够正确回滚。

通过以上内容，你应该能够更好地理解和使用 Android 上的 SQLite 事务，确保数据库操作的安全性和一致性。


SQLite提供了完整的ACID事务支持，可以将多个操作合并为一个，本节我们将了解在Android平台上的使用方法。
...
<介绍一下每个方法以及成功/失败的跳转逻辑>



规则：
每个连接是串行化的，如果多个事务同时执行，最先开始的事务阻塞其他事务。
Android中的SQLite事务是最高隔离级别，一个事务开启后其他事务所在线程将被阻塞，直到首个事务执行完毕后才会继续运行，不支持多个连接并发
beginTransaction()


dbHelper.getDB().beginTransactionNonExclusive() ：对于多个连接（Helper实例），NonExclusive允许其他连接读取数据（当然是事务提交前的状态，不会读到未提交事务。）,一般数据库Helper实例是单独的，并且程序独享自己的数据库，不存在跨进程的情况，因此上述场景出现较少，只需要了解即可。
