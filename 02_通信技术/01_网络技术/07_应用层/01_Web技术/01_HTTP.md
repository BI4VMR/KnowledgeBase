...
## Basic认证
### 简介
基本认证是一种简单的认证方式，客户端将会明文发送用户名称和认证口令至服务端。



用户信息容易泄漏，每个url都要携带认证信息，不灵活，不支持跨域，可扩展性差，通常仅用于Jenkins、Tomcat管理页面等内网应用。
...
响应头中的 `WWW-Authenticate` 属性指明了服务器要求的认证参数，其中"Basic"表示认证方式为Basic认证；"realm"表示
