...
超文本传输协议(Hypertext Transfer Protocol, HTTP)是Web领域的基础协议，客户端与服务端通过该协议进行交互：客户端向服务端发送HTTP请求资源；服务端则将网页代码或数据封装在HTTP返还给客户端；最终客户端渲染网页或解析数据并展示给用户。

浏览器(Browser)是最早被广泛应用的Web客户端，通常用于PC，常见的浏览器包括：Google Chrome、Mozilla Firefox和Safari。用户在浏览器中输入网站地址，网站服务器返回HTML代码给客户端，浏览器按照HTML内容在屏幕上排布文本、图片等资源，并设置字体、字号、图片尺寸等属性，以供用户进行阅读。

应用程序(Application)是比较特殊的Web客户端，它们通常不会直接传输HTML并渲染，而是传输JSON、XML等文本信息，由程序代码绘制界面进行展示。
...
浏览器非常适合快速访问和浏览互联网内容，而应用程序则能提供更为丰富和个性化的体验。随着技术的发展，两者的界限有时会变得模糊，例如Progressive Web Apps (PWAs)就是试图结合两者优点的一种尝试。
...
🔷 静态网页技术
    ...
    HTML (HyperText Markup Language): 用于构建网页结构的基本语言。
    CSS (Cascading Style Sheets): 用来描述HTML文档的呈现样式，如布局、颜色和字体等。
    JavaScript: 一种脚本语言，用于实现网页上的交互功能。近年来，随着各种框架和库的发展（如React、Vue.js和Angular），JavaScript已经成为前端开发的核心。

制作静态网页框架、动效的工作被称为“前端开发”
