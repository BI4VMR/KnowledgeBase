...
应用程序与服务器之间有多种可用的交互方式，我们可以选用任意类型的传输协议实现数据与指令交互，但如果采用HTTP协议，则有助于减少工作量，例如：某个软件产品同时支持浏览器与应用程序，如果采用HTTP协议，服务端只需要维护一套对外接口即可，不必为浏览器和应用程序单独维护对外接口。

如果我们只需要在浏览器与应用程序之间选择一种作为产品客户端的形态，可以参考下文列表的描述：

- 浏览器是通用的，我们无需为每个硬件平台专门开发浏览器，只需处理不同浏览器的兼容性问题，因此客户端的开发与维护成本相比应用程序更低。
- 浏览器是标准的，能够调用的系统能力较为有限，例如：传输体积较小的文件、访问摄像头与麦克风等，因此只适合功能较为简单的产品；如果产品需要访问硬件驱动、传感器等，我们只能通过应用程序实现。

🔷 静态网页技术

静态网页技术用于描述网页形态 制作静态网页框架、动效的工作被称为“前端开发”

- HTML(HyperText Markup Language) : 用于描述网页结构的基本语言，包括元素声明、布局定义。
