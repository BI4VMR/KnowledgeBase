...
- JavaScript : 脚本语言，用于控制网页中的元素，实现用户交互逻辑。
...
静态网页的样式与内容是耦合的，如果我们需要新增内容，就要制作对应的页面并上传至服务器，冗余代码较多、维护难度较大。随着时代的发展，静态网页已经无法满足人们的需求了，因此出现了动态网页技术。

动态网页的内容是可变的，我们预先定义网页的样式但不填充内容，然后将内容保存到数据库中；当客户端发起请求时，动态网页服务程序将会从数据库中取出对应的数据，并插入到样式模板中，最后将网页返回给客户端。

常见的静态网页服务程序包括：Microsoft ASP .Net、Java Tomcat、Java Netty、Python Django等，编写动态网页服务程序的工作被称为“后端开发”，开发者需要熟练掌握上述编程语言及对应的框架。
