...
<!-- TODO ![进程间的数据传递](./Assets-Parcelable接口/概述-进程间的数据传递.jpg) -->

</div>

Context类本身是一个抽象类，它有两个具体的实现类ContextImpl和ContextWrapper。其中ContextWrapper类只是一个外层包装，其构造方法必须包含一个Context实现对象，它有三个具体实现。其中ContextThemeWrapper包含与主题相关的接口，典型的实现类为Activity；而Application和Service则没有界面，不需要主题。
...
当我们初始化Dialog时，只能使用Activity，因为系统不允许弹窗随意出现，必须依附于某个Context。
...
|   用途/类型    | Application | Activity | Service |
| :------------: | :---------: | :------: | :-----: |
|    显示弹窗    |      ❎      |    ✅     |    ❎    |
|  启动Activity  |   不建议    |    ✅     | 不建议  |
|    渲染布局    |   不建议    |    ✅     | 不建议  |
...
2. 当工具类封装了其他需要Context的方法调用时，最好使用参数透传Context，不要使用全局变量。
3. 当工具类需要持有静态变量时，需要调用 `getApplicationContext()` 方法获取Application，防止使用者误操作传入Activity或Service。
