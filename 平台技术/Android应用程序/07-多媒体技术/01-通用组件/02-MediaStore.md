<!-- (TODO)
# 概述
MediaStore是Android Framework提供的媒体库服务，系统会在开机或存储介质挂载时自动扫描特定目录下的文件，并根据媒体类型检索元数据（例如音乐文件的标题、艺术家、专辑等），然后将这些数据存入SQLite数据库中，如此一来，其它程序不必各自维护媒体数据，可以直接利用系统提供的公共接口。
