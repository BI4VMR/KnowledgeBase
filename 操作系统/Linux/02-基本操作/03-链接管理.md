...
# 创建test的硬链接，名称为"hardlink"。
[root@Fedora ~]# ln test hardlink
# 创建test的硬链接，名称为"softlink"。
...
我们从上述示例中可以发现："test"和"hardlink"的inode值相同，指向同一块数据区域，本质上是同一个文件，"softlink"的inode值与它们不同，是独立的文件，并且"ls"输出结果中使用"->"指出了它的链接目标。
