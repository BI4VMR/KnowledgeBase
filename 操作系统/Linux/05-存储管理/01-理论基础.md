# 分区表
一块物理磁盘可以划分为多个逻辑分区(Partition)，操作系统将每个分区视为独立的磁盘并对其进行读写操作。各个分区的元数据记录在分区表(Partition Table)中，分区表位于磁盘可用空间的起始部分，x86体系主要使用MBR和GPT分区表。
...
由于MBR设计年代久远，分区容量字段限定为4字节，无法创建2TB以上的分区，因此被GPT淘汰。

## GPT
GUID分区表(Globally Unique Identifier Partition Table, GPT)是UEFI规范的子集，其支持最多128个主分区，且单个分区容量可达18EB，另外还支持分区表校验与备份。GPT相比MBR具有更多的优点，目前正被广泛使用中。
...
## XFS
XFS是Silicon Graphics公司开发的高性能日志式文件系统，被RHEL7作为默认文件系统。XFS基于分配组的设计使其非常擅长并行IO，日志记录功能使其不容易丢失数据，且最大可支持18EB的存储空间，是一种先进的现代文件系统。
...
微软公司还推出了exFAT文件系统，这是一种非日志式文件系统，读写动作较少，适合闪存设备使用，但无法保证数据完整性。鉴于当前闪存设备较为廉价，不建议使用。
...
> 🚩 提示
>
> 磁盘名称、分区编号与硬件拓扑无关，仅受内核加载顺序影响。"sda"不一定对应主板上的第一个插槽，磁盘存在分区"sda3"时也不能表明共有4个分区。

# 挂载与卸载
## 挂载设备
Linux系统没有盘符的概念，我们在使用已格式化的分区之前，需要执行"mount"命令将其关联至某个目录，这个过程称为“挂载”，目录称为挂载点，挂载完成后读写该目录即等同于读写对应的磁盘分区。

挂载命令的语法为： `mount {选项} [设备文件] [挂载点]` ，其常用选项参见下文：

🔷 `-a`
<br />
根据"/etc/fstab"配置文件，将未挂载的磁盘自动挂载。
...
🔷 `-o`
<br />
指定额外的挂载配置。本选项支持的参数见下文表格，若需要配置多个参数，请使用逗号分隔。
...
|    fmask    |              指定文件权限掩码              |
|    dmask    |              指定目录权限掩码              |
|  usrquota   |      启用对磁盘配额模式的支持（用户）      |
|  grpquota   |     启用对磁盘配额模式的支持（用户组）     |
...
🔷 `-t`
<br />
指定文件系统的类型。通常系统可以自动侦测磁盘文件系统，不必指明该参数，若挂载过程中出现异常，才需要手动设置。
...
## 卸载设备
文件"/etc/mtab"中记录当前系统已挂载的存储设备，如果我们不再使用一些存储器，需要执行 `umount` 命令将其卸载，操作成功后方可将设备物理移除。此命令输入参数可以为挂载点或磁盘分区，语法为 `umount {选项} [磁盘分区|挂载点]` 。
...
>
> 对于此类设备可添加"_netdev"选项，系统将在联网后再执行挂载，避免阻滞启动过程。
