...
RPM基于"/var/lib/rpm"目录中的数据库维护软件信息，操作已安装的软件时，仅需指定名称，不必指定版本号等信息。

## 安装软件
"rpm"命令的选项为“主要动作+附加选项”，附加选项可以有多个，但主要动作（安装、卸载等）仅能指定一个，不可冲突。安装动作记作"-i"，通常携带选项“显示详细信息(-v)”和“显示进度(-h)”，故安装软件包的语法为： `rpm -ivh [RPM文件]` 。
...
## 卸载软件
卸载动作记作"-e"，语法为： `rpm -e [软件名称]` 。卸载软件时，若该软件包被其它程序依赖，则显示消息并终止操作，此时可以添加"--nodeps"强制执行，但这可能导致其它程序缺少依赖而出错，需谨慎操作。
...
若需要降级某个软件包，可以在安装命令后添加"--force"选项强制安装旧版本的RPM包，写作 `rpm -ivh --force [旧版RPM文件]` 。
...
我们可以添加"-i"选项以查看特定软件包的详细信息，包括版本号、适用架构、安装日期、官网等。
...
若需要查看本地RPM包的信息，可以添加"-p"选项，写作 `rpm -qi -p [RPM文件路径]` 。
...
执行 `yum list` 命令可列出软件仓库中所有可用的软件包， `yum search [关键字]` 可以搜索名称或简介中包含该关键字的软件包。
...
### 软件组管理
YUM内置了一些软件包的集合，可以同时管理一组软件包，此功能对于特定的使用场景非常有效，下文中的代码块列出了软件组管理的相关命令。
...
每个配置文件可以包含若干软件源，每个软件源以方括号包围的名称开头，包含若干参数项：
...
镜像列表指定了一系列镜像站地址，系统将会从中选择一个使用；源路径则指向特定的服务器或文件，二者选择一项即可。
...
## 下载安装包
为安装命令添加"--downloadonly"选项可以仅下载软件包，而不进行安装。对于离线环境，可以通过已联网的设备下载软件包与相关依赖，并拷贝至离线设备进行安装。
...
对于已安装的软件包，YUM不会自动下载其依赖，此时我们可以使用软件包"yum-utils"中的"yumdownloader"工具，操作更为便利。
