...
## DSCP
区分服务码点(Differentiated Services Code Point, DSCP)由RFC 2474定义，使用ToS字段的高六位，其余位固定为0。DSCP定义了3大类流量，比IP优先级分类粒度更细。

CS
若ToS字段除了高3位均为0，则属于类选择器码点(ClassSelector Codepoints,CS)，其意义与IP优先级相同，CS0-CS7对应IP优先级的0-7。取值为0时提供尽力而为的服务，取值非0时尽可能地提供优先转发服务。
