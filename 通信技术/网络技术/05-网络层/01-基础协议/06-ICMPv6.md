...
# 邻居发现机制
## 简介
邻居发现协议(Neighbor Discovery Protocol, NDP)取代了ARP协议，在以太网环境中，获取其它节点MAC地址时不再广播发送请求报文，而是使用组播点对点通信，不会占用无关节点的处理能力，提升了通信效率。

NDP由RFC 2461定义，使用ICMPv6的5种消息实现了地址解析、重复地址检测、邻居状态检测、路由器发现和重定向功能。

## 工作流程
IPv6节点会为所有IPv6单播地址各加入一个组播组，前缀固定为FF02:0:0:0:0:1:FFXX:XXXX/104，X则对应接口标识符中的后24位。
...
被请求者加入了被请求节点组播组，能够收到NS请求报文，将会发送邻居通告(Neighbor Advertisement Message,NA)报文(Type 136)给请求者，其中包含自身的MAC地址。

图 5-17 邻居请求报文
PC1欲发送数据包给PC2，需要获得其MAC地址，首先发送NS报文进行查询，目的地址为PC2的被请求节点组播地址(FF02::1:FF00:0002)。若交换机不支持组播，会将该报文广播到其它接口。PC2接收该报文，PC3根据组播地址在链路层的映射关系，发现自身没有加入3333:FF00:0002这个组播组，直接丢弃报文，无需解开三层封装并比较。

图 5-18 邻居通告报文
PC2单播发送NA报文给PC1回应自身的MAC地址，同时记录下NS报文中PC1的MAC地址，此时双方知晓了对方的MAC地址。
ARP请求报文的目的MAC为FF-FF-FF-FF-FF-FF，所有节点都要解除二层封装，根据目的IP地址判断是否需要丢弃报文，NDP使用组播之后，其它节点根据组播MAC映射就可以直接判断是否需要丢弃报文，效率更高。若使用智能型交换机，还可以识别终端加入的组，只将请求报文发给对应的节点，不用广播给其它无关节点，进一步提升通信效率。
使用以下命令可以查看IPv6邻居信息：
...
NDP协议不仅能够发现邻居，还能探测邻居的状态。
                • IPv6邻居状态机
    • Incomplete
未完成状态，是发送NS报文但未收到NA报文时的状态。
    • Reachable
可达状态，收到NA报文后的邻居状态，若10秒后还未收到NA报文，删除条目。
    • Stale
...
Cisco(config-if)#ipv6 nd dad attempts [探测报文数量]
            2.3.7   无状态地址自动配置
IPv6支持无状态地址自动配置(Stateless Address Autoconfiguration,SLAAC)，此功能集成在ICMPv6协议中，无需DHCP就能完成地址配置，使配置更加便捷。
                • 工作流程
网段内的路由器默认每隔200秒发送路由器通告(Router Advertisement,RA)报文(Type 134)，向其它节点通告本网段的前缀信息，其它节点使用EUI-64形成接口标识符，就可以组成自己的IPv6地址。
当一个接口配置了SLAAC IPv6地址后，将会主动发送路由器请求(Router Solicitation,RS)报文(Type 133)请求网络前缀，路由器收到后不用等待200秒，立即回复RA报文。如果该节点是一台终端主机，还会将通告前缀的路由器设置为自身的默认网关。
在Cisco设备上，开启IPv6路由功能后将会发送RA报文，可以使用以下命令关闭：
Cisco(config-if)#ipv6 nd suppress-ra
                • SLAAC地址生命周期

图 5-20 SLAAC地址生命周期
刚获取SLAAC地址时需要进行DAD检测，此阶段地址不可用；若没有发现地址冲突，地址可用。RA报文中含有有效时间和推荐时间两个字段，收到新的RA报文时计时器将会重置。推荐时间结束后，地址进入不建议使用状态，此时可作为目的地址，但不推荐作为源地址使用，除非不得不用（详见RFC 3484）；有效时间结束后，地址将被删除。
                • 通告额外的IPv6前缀
