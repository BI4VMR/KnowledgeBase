...
# 报文类型
## Hello
Hello报文用于建立与维护邻接关系，发送到组播地址224.0.0.5，TTL值为"1"。
...
指定路由器(DR)的IP地址，如果为0.0.0.0，表示未选举DR。

🔷 Backup Designated Router

备份指定路由器(BDR)的IP地址，如果为0.0.0.0，表示未选举DR。
...
# 度量值
OSPF中使用“路径开销(Cost)”作为度量值，路由器收到拓扑信息报文后，读取其中的度量值，再计算出接收接口的度量值，两者之和即为该网段的路径开销。
...
1.4.10  OSPF网络类型
 逻辑网络类型
OSPF协议为了适应不同的数据链路层环境，定义了不同的逻辑网络类型，不同类型的网络有各自的特点，与物理介质类型无关，可以根据需要修改。
表 5-18 OSPF网络类型
...
 优化方法
在只有两台设备直连的以太网链路上，无需选举DR/BDR，因此我们可以将链路两端接口的逻辑网络类型设为点到点，以此加快路由收敛速度。
...
# 前缀列表
前缀列表(Prefix-List)用于匹配路由条目的前缀、掩码，可以对特定的路由条目进行放行或拦截，达到控制选路的目的。在路由条目较多时，前缀列表较ACL有更优的性能。
 使用方法
...
# 被动接口
有些接口需要宣告进OSPF进程中，但不希望让下联设备收到OSPF报文，例如连接终端设备的业务VLAN或环回接口，此时可以配置被动接口特性，使其不在收发OSPF报文。
 配置被动接口
...
# 虚链路
虚链路(Virtual-Link)可以穿越非骨干常规区域形成一条逻辑链路，将不规则区域与骨干区域相连，从而实现LSA正常传递。
 
图 5-44 虚链路示意图
虚链路是一种临时措施，用于解决不合理的OSPF设计，默认属于区域0。
建立虚链路后，被穿越区域的ABR将会产生一类LSA，包含类型为Virtual Link的链路状态信息，LS-ID为虚链路对端的路由器ID，LS-Data为本地接口地址，对端收到这些LSA后将其标记为永不超时。
 使用限制
 一般在同一个区域内的两台ABR上进行部署。
 只能穿越常规区域，不能穿过骨干区域与特殊区域。
 相关配置
 在需要穿越区域两端的ABR上配置虚链路
...
 典型应用
以下拓扑中，各路由器ID为其序号。区域2无法直连到骨干区域，此时可以在区域1中连接骨干区域和区域2的两台路由器上建立虚链路，实现区域2与骨干区域的连接。
...
1.4.20  LSDB过载保护
该特性用于防止性能较差的路由器收到过多的LSA导致CPU过载。
 相关配置
Cisco(config-router)#max-lsa [最大LSA数量] {预警比例} {warning-only} {ignore-time [时间/分钟]} ignore-count [次数] reset-time [时间/分钟]
参数	含义
预警比例	达到设定数值的百分之几时发出警告，默认为75%
warning-only	仅发出警告
ignore-time	持续过载达到该时间后，重置邻接关系
ignore-count	重置操作达到该次数后，不再恢复邻接关系
reset-time	邻接关系保持该时间后，重置ignore-count计数
...
有时为了人工控制选路，可以在LSA传入接口上设置需要增加的度量值。
增加接口度量值
