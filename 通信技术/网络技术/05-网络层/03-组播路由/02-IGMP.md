...
![IGMPv1报文格式](./Assets-IGMP/报文格式-IGMPv1报文格式.jpg)
...
组播组地址，各节点发送时根据需要填入内容。

# 报文类型
## 通用组查询报文
组查询报文(General Query)由查询器向网络发出，用于查询组播组是否存在以及维护组成员关系，Group Address字段置0。
...
## 查询器选举
当网络中存在多个路由器时，为了节约带宽资源，需要选出一个路由器作为查询器。在IGMPv1中，查询器由PIM协议的DR担任；较新版本中则可以自行选举查询器。
