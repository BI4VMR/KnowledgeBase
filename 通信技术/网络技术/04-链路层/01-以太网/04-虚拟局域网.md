# 概述
虚拟局域网(Virtual Local Area Network, VLAN)是一组逻辑上的设备和用户，这些设备和用户不受物理位置的限制，可以根据功能、部门等因素将它们组织起来，相互之间可以直接通信。不同的VLAN之间不能直接通信，要借助于路由器或者三层交换机。VLAN是逻辑子网，也是独立的广播域，VLAN技术的应用使组网更灵活，更便于管理。
...
PVID(Port VLAN ID)是交换机端口的一种属性，进入端口的以太帧会被打上PVID对应的VLAN Tag，端口默认的PVID为1。
                • VLAN Trunk
VLAN中继(Trunk)使不同交换机的同一VLAN成员能够相互通信。将交换机之间的互联链路设为Trunk模式后，其可以承载多个VLAN的流量。
设备收到数据帧后，给帧添加接口所属VLAN的标记信息，查询CAM表，若目的MAC地址关联到Trunk接口，就发送到Trunk链路上；接收端设备读取标签中的VLAN ID后，把IEEE 802.1q帧转换为普通以太网帧（删除标记），再转发给相应的VLAN成员。

图 4-14 Trunk链路

# 中继封装协议
                • IEEE 802.1q
IEEE 802.1q是公有协议，具有更小的开销，但改变了帧头格式，需要重新计算校验和；其支持的VLAN ID范围：1-4094。
                • ISL
ISL是思科私有协议，给数据帧添加26字节的帧头，不改变原始帧头格式，其支持的VLAN ID范围：1-1005，改进后支持1-4094的全部VLAN，该协议目前已被淘汰。
...
<!-- TODO ![Ethernet Ⅱ的帧结构](./Assets-概述/Ethernet2的帧结构.jpg) -->

</div>

🔷 EtherType

表示封装协议类型为802.1q，取值恒为0x8100。如果不支持802.1q的设备收到这种数据帧，因为无法识别，会将其丢弃。
...
该字段用于区分以太网帧、FDDI帧或令牌环网帧，以太网帧中取值为0。

🔷 VLAN ID

VLAN号，取值范围[0,4095]，其中0表示优先级帧，4095是保留值。
...
这就是VTP协议经常与端到端VLAN模型结合使用的原因，只需在VTP服务器上添加或修改VLAN，VTP会将变更自动同步到整个管理区域。

这种模式有以下优点：

<!-- 任何用户都可以从任何交换机访问其资源，如果部署了VMPS服务器，用户插入任何交换机端口都能被自动分配到正确的VLAN。

这种模式有以下缺点：

首先，端到端VLAN在大型网络中很难维护。如果没有在所有交换机上运行VTP服务，则必须手动为每台交换机配置正确的VLAN。其次，如果每个交换机都需要配置所有VLAN，这意味着它们也必须跨越网络的汇聚层和核心层，所有VLAN上的广播流量也必须遍历核心，这可能导致严重的性能问题。 -->
...
<!-- 本地VLAN配置意味着简化的VLAN故障排除和更少的生成树设计考虑。通过分布交换机上的SVI，性能也得到了改善——创建更小的广播域。

这种模式有以下缺点：

本地VLAN模型几乎没有什么缺点。如果您习惯了VTP自动传播VLAN更改，那么它将会变成需要手动配置的过程。但这一变化与大规模端到端VLAN部署相比，所需的总体管理开销仍然减少了。 -->
