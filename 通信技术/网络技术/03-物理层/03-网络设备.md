...
![交换机外观示例](./Assets-网络设备/外形-交换机外观示例.jpg)
...
![路由器外观示例](./Assets-网络设备/外形-路由器外观示例.jpg)
...
AUI端口是一种D型15针接口(DB-15)，是用来与粗同轴电缆连接的接口，在令牌环网或总线型网络中是比较常见的接口。路由器通过粗同轴电缆收发器实现与10Base-5网络的连接，也可以借助AUI转RJ-45模块实现与10Base-T以太网的连接。

# 硬件组成
网络设备是一种专用型计算机，其硬件组成部分如下表所示：
...
![思科设备命令提示符](./Assets-网络设备/思科命令行-思科设备命令提示符.jpg)
...
![华为设备命令提示符](./Assets-网络设备/华为命令行-华为设备命令提示符.jpg)
