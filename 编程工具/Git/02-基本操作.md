...
方法1相当于将两次修改合并后创建一个提交；方法2每次修改均会创建提交，我们可以根据需要进行选择。而方法3则是错误示范，因为Git管理的是文件差异，而不是文件本身，由于第二次修改后没有将最新变更添加至暂存区，相当于仅提交了第一次的修改。
...
该操作相当于清除较新的提交记录，但保留所有变更并移入工作区。当我们希望合并提交或重建提交时可以使用上述的两种方式。
