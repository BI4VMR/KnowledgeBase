...
Flow 接口有多种具体实现，总体上可分为“冷流”与“热流”两种类型，二者的行为差异很大，我们应当根据应用场景选择合适的实现。

冷流包含开发者预先定义的一系列逻辑语句，当接收者开始从冷流监听数据时，这些逻辑语句才会被执行并发送数据。

冷流类似于包含回调接口的方法，适用于封装需要异步多次返回结果的任务，例如：文件下载、数据库查询等。

热流不包含逻辑语句，而是从外部接收数据更新，并将数据变化事件分发给所有监听者。

热流类似于广播电台，适用于事件通告与状态共享，例如：消息推送、 UI 状态管理等。
...
Flow 实例的 `collect(collector: FlowCollector<T>)` 方法用于监听 Flow 产生的数据，这是一个挂起函数，监听者需要在协程作用域中调用它；该方法的唯一参数 `collector` 用于指明每次收到数据时的处理逻辑，默认参数 `it` 表示当前数据值。 Flow 的逻辑代码中每调用一次 `emit()` 方法， FlowCollector 接口就会被回调一次，二者是一一对应的。
...
热流运行后永不停止，因此 collect 语句后不能放置任何其他语句，它们永远不会被执行。
...
测试协程发出了消息，但监听协程没有任何响应，这说明sharedflow不会重放历史消息，监听者只会收到注册监听后发出的新消息。


第二步，在上述代码的基础上，继续发送数据，并观察控制台输出
...
此时测试线程发出的所有消息，监听协程内都有对应的日志输出，二者是对应的。
...
- 忽略重复值：当外部组件向 StateFlow 投送数据时， StateFlow 将当前缓存的值与外部传入的值进行比较，若二者相同则不通知观察者，仅当新旧数据发生变化时才会通知监听者。
- 丢弃中间值：如果短时间连续发送多个状态，只会推送最后一个状态给接收者。这在状态管理场景中无问题，因为只需确保最终状态一致，监听者不关心中间过程。但如果错误地将StateFlow用于某些猝发高频事件通知场景，会导致事件丢失。
