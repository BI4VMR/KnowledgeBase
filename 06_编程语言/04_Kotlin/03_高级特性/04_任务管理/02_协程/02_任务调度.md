...
协程的树状结构有利于反映任务之间的关系，更好地管理任务与资源，详见相关章节： [🧭 结构化并发](04_协程环境.md#结构化并发) ，目前我们仅需了解如何在协程任务中实现并发即可。
...
在前文示例中，为了确保主线程等待协程任务执行完毕再退出，我们需要使主线程休眠固定的时长；而实际应用中我们难以预估异步任务的具体耗时，因此使用 `runBlocking()` 方法更为合适。
