# 概述
截止至本节内容，我们的多数示例代码通过 `CoroutineScope(Dispatchers.Default).launch { ... }` 语句启动协程，每次都会新建一个CoroutineScope实例，这些协程相互独立，没有产生关联。本节内容将会讨论CoroutineScope的相关知识，以结构化方式管理一组具有相关性的协程。
...
Kotlin针对一些通用场景提供了预设作用域，无需手动创建Ctx，我们可以直接使用全局变量调用它们并启动协程
