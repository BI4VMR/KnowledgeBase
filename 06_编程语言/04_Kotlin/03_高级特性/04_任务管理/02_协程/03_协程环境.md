...
- CoroutineScope : 协程作用域，作用域与上下文一一对应，我们可以通过作用域启动与取消协程，这些协程的行为将按照上下文配置进行操作。
...
对于较为独立的底层模块，例如数据库模块、HTTP模块等，我们可以不指定协程环境，将所有方法都设为 `suspend` 方法，由上层调用者（可能是UI组件或业务聚合管理类）决定这些方法需要执行的环境。
...
例如Android的页面可以使用lifecyclescope，当界面关闭时任务自动取消。

对于业务聚合管理类，一般需要调用多个子模块的接口，因此需要内置协程环境，组织各个协程任务。
...
此时我们可以将调度器作为组件构造时的参数，并提供默认值，业务代码构造实例时可以不指定调度器，此时使用默认的调度器，测试代码可以指定测试调度器，以便控制协程任务与断言的执行顺序。

对于业务方法也不需要各自新建作用域，而是应当使用统一的作用域，当业务组件使用完毕后，外部管理者可以调用 `destroy()` 方法同步取消所有协程。
