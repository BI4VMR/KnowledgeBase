...
`CoroutineScope()` 方法用于创建协程环境，其中 `Dispatchers.Default` 是框架内置的调度器，指定了任务将在子线程中运行， `launch()` 方法也被称为“协程构建器”，用于创建任务， `launch {}` 内部的语句即协程任务，也被称为“协程体”。

此处协程任务中的 `getThread()` 方法用于获取工作线程名称， `getTime()` 方法则用于获取语句执行的时刻，具体的实现已省略，详见本章示例工程。

当协程启动后，我们调用 `Thread.sleep()` 方法阻塞主线程直到协程执行完毕，这是因为JVM平台中的协程基于线程实现，主线程执行完毕则进程结束，所有非守护线程都会立刻终止。此处的实验环境为Java SE平台，协程任务运行在非守护线程中，故需要采取等待协程执行完毕的措施；若在Web Server、Android等环境中进行实验，则无需调用 `Thread.sleep()` 方法，因为它们的主线程会无限循环，不会自行终止。为了减少篇幅，后续示例代码均省略 `main()` 方法与 `Thread.sleep()` 方法调用。
...
## 挂起函数
对于可复用的代码片段，我们通常会将其定义为方法。此处我们尝试将前文“示例一”中的协程任务定义为 `task()` 方法。
...
上述语句将会导致编译错误，这是因为“挂起”特性是Kotlin协程专有的，非协程环境无法使用该特性，此处的延时方法 `delay()` 就是一个会导致“挂起”的语句，只能出现在协程任务中。

如果一个方法中包含可能导致“挂起”的语句，我们必须在方法声明语句中添加"suspend"关键字，标记本方法只能在协程任务中使用。

🟠 示例二

在下文示例中，我们定义一个挂起函数，并在协程任务中调用它。
...
在下文示例中，我们开启一个协程，并尝试访问协程任务的三个公开属性。
...
在上述场景中，当页面被关闭时，我们应该取消相关的协程任务，以便节约资源并避免发生错误。
...
在下文示例中，我们启动一个协程，并在任务进入挂起状态时从主线程发起中断请求。
