...
# 基本应用
CoroutineScope表示协程环境，我们可以通过该类创建与管理协程。
...
当协程启动后，我们调用 `Thread.sleep()` 方法阻塞主线程直到协程执行完毕，这是因为实验环境为Java SE平台，主线程运行完毕则整个进程终止，故需要采取等待协程执行完毕的措施；若在Web Server、Android等环境中进行实验，则无需调用 `Thread.sleep()` 方法，因为它们的主线程会无限循环，不会自行终止。为了减少篇幅，后续示例代码均省略 `main()` 方法与 `Thread.sleep()` 方法调用。
