...
下文示例代码展示了接口回调风格的模拟网络请求API定义与使用方式。

🔴 示例一

我们首先定义回调接口，并在模拟网络请求的方法中调用它。
...
接口定义完毕后，调用者需要传入NetCallback实现类的实例，并在结果返回时执行对应的操作（例如更新界面），此处我们只是简单的将回调结果输出到控制台上。
...
在下文示例中，我们使用 `suspendCoroutine()` 方法将回调接口 `request()` 转换为挂起函数。
...
## 反馈异常状态
NetCallback接口有“请求成功”和“请求失败”两种情况，由于挂起函数只能有单个返回值，已经被我们用来反馈成功状态，因此我们还需要通过异常机制反馈失败状态给调用者。
...
为前文“示例二”中的 `requestSuspend()` 方法添加中断处理功能。
...
接下来，我们创建一个协程任务调用挂起函数 `requestSuspend2()` ，并在主线程中发起中断请求。
...
使用 `runBlocking()` 方法将前文“示例二”中的挂起函数 `requestSuspend()` 转换为接口回调形式。
