...
## 处理中断请求
在前文示例中，通过 `suspendCoroutine()` 方法构建的挂起函数并不支持中断请求，外部调用 `cancel()` 方法不会有任何反应，协程任务会继续执行直到结束为止。

为了支持中断任务的请求，我们可以将 `suspendCoroutine()` 方法替换为 `suspendCancellableCoroutine()` 方法，它们的使用方法是类似的，后者额外增加了中断处理功能。
