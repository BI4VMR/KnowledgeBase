...
UtilsClass类中有一个静态方法，被调用后将会在控制台上输出一行消息。
...
我们可以通过 `Mockito.mockStatic(<目标类的Class>)` 方法获取目标类的静态Mock配置对象，然后通过该对象配置Mock行为、或对静态方法进行验证。静态Mock配置对象使用完毕后需要释放，防止污染其他测试用例，因此我们通常将其放置在 `try-with-resources` 语句块中。
