...
在Java语言中， `new` 关键字表示创建一个对象，其后是目标类的构造方法（即 `<类名>()` ），我们可以声明一个变量引用该对象，以便后续再次访问对象。
...
## 局部变量
我们在代码块 `{}` 中声明的变量被称为“局部变量”，这种变量的作用域与全局变量相反，仅限于声明变量的代码块内部。局部变量通常用于保存临时性的数据，当代码块中的语句执行完毕后变量就会被清除，基本数据类型变量所占用的内存将被立刻回收，引用数据类型变量所占用的内存则由垃圾收集器控制回收。
