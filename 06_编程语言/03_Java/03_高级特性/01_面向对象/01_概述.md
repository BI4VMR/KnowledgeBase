...
## 基本应用
我们首先定义一个Person类描述“人类”，它拥有姓名、年龄、性别属性，以及“说话”方法。
...
“对象”是程序运行时根据“类”动态产生的实体，例如：“张三”的年龄是18岁，性别为“男”；“李四”的年龄是20岁，性别为“女”。我们在测试类中创建两个对象，并通过访问属性描述“张三”和“李四”。
...
## 空值对象
引用类型变量存储的是指向堆内存的地址，对象就是一种引用类型变量，如果一个对象并未指向任何堆内存，它的值就为"null"，也被称为“空值”。

空值对象产生的原因可能是：全局变量未经初始化、程序员主动将变量赋值为"null"、方法的返回值为"null"等，其中部分情况是编码遗漏、时序不当这类错误；而另外的部分情况则是"null"具有特殊含义，例如：从数据库中查询学生信息时，返回非空Student对象表示存在记录；返回空的Student对象表示不存在记录。

如果我们试图访问空值对象中的变量或方法，就会产生空指针异常："NullPointerException"，这种异常也被简称为NPE，经常导致程序崩溃，详见下文示例代码。
...
Exception in thread "main" java.lang.NullPointerException
	at net.bi4vmr.study.base.TestClass.main(TestClass.java:17)
```
...
当我们没有声明构造方法时，类中默认存在一个无参数、方法体为空的构造方法。以前文的Person类为例，该方法为 `Person(){}` ；我们在测试类中创建Person对象时，"new"关键字后跟随的部分即为构造方法。
...
在上述示例代码中，我们需要使用"this"关键字为全局变量完成赋值，这是因为构造方法的形参与全局变量同名，此时型参的优先级高于全局变量，语句"name"即表示型参 `name` ；"this"关键字表示“当前类的对象”，语句"this.name"表示全局变量 `name` ，语句 `this.name = name` 表示将型参 `name` 的值赋给全局变量 `name` 。

随后，我们在测试类中调用有参构造方法，一次性给三个属性赋值，完成对象的初始化。
...
## 局部变量
我们在代码块中（例如：方法体内部）声明的变量被称为“局部变量”，这种变量的生命周期仅限方法内部，当方法运行完毕后就会立刻被JVM回收。由此可见，“局部变量”的作用域仅限其所在的代码块，一般用于保存临时性的数据。
...
在上述示例代码中，方法 `function1()` 内部定义的局部变量 `a` 无法被方法 `function2()` 访问，其中的 `System.out.println(x)` 语句会导致编译错误。

# 静态变量与方法
被"static"关键字修饰的成员变量或方法，通常称为“静态成员”或“类成员”，它被类的所有对象所共享。静态成员的生命周期跟随其所属的类，当JVM第一次加载某个类时，其中的静态变量将被初始化并分配资源；直到进程结束时，静态变量才会被释放。
...
我们可以通过类或对象访问静态变量与方法，鉴于它的生命周期跟随类，我们更习惯使用类名进行访问。
...
对象1将静态变量"num"的值修改为"-1"后，对象2读取到的值也为"-1"，说明该变量确实被类的所有对象所共享。

静态方法可以访问静态变量与其它静态方法，但无法访问非静态成员。我们常用的 `main()` 方法就是一个静态方法，JVM不需要新建测试类即可直接执行其中的语句。

# 初始化块
初始化块提供了一种初始化成员变量的方式，我们可以在其中进行数据赋值操作。一个类中可以包含多个初始化块，当我们创建类的实例时，系统就会按照书写顺序依次执行这些代码块。

如果我们使用"static"关键字修饰初始化块，它就会成为静态初始化块，类加载时将被最先执行，并且只会执行一次；静态初始化块内部只能初始化静态成员变量。
...
当程序运行时，测试类被JVM加载，静态初始块首先被执行，且只会执行一次。随后程序运行至 `main()` 方法，当创建第一个对象时，初始化块、构造方法依次被执行；当创建第二个对象时，初始化块再次被执行。

# Object类
## 简介
Object类是 `java.lang` 包中的核心类，它是所有类的顶级父类，其他的类都继承自Object。

Object类拥有一些实用的方法，后文章节将对它们进行详细的介绍。

## 输出对象信息
Object拥有 `String toString()` 方法，该方法能够以字符串形式展示对象的详情，我们可以用它输出日志以进行调试。

`toString()` 方法默认返回对象在内存中的哈希值，我们自定义的对象通常需要重写该方法，输出重要的成员变量信息。

以前文的Person类为例，我们可以重写该方法，并输出实例的名称、年龄、性别。
