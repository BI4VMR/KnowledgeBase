# 简介
我们将现实世界中的事物抽象成为“类”，通过属性与方法描述事物的特征与行为，有些属性与方法仅会在类的内部使用，它们不必暴露给外部，这就是封装(Encapsulation)的思想。

各个模块划分职责后，仅需要暴露外界所关注的属性与方法，其他属性与方法应当对外隐藏，防止意外的修改动作造成非预期结果。

# 类的封装
Java使用“包”的概念管理源代码，并解决同名类的冲突问题。

若干类组成的功能模块可以被封装成为“包”，包名的格式通常为 `<反写域名>.<模块名称>` ，例如组织域名为 `test.org` 、模块名称为 `util` ，则对应的包名应当为 `org.test.util` 。
...
## BO
业务对象(Business Object, BO)用于组合简单的PO，并且包含部分业务逻辑、对DAO和外部接口的调用等。BO只会在超大规模工程的业务逻辑模块中出现，一般规模的工程只使用Service管理业务逻辑就能够满足要求了，不需要单独划分BO。

# 内部类
## 简介
内部类(Inner Class)是指声明在一个类体中的类，与之对应的，包含内部类的类，被称为外部类。

内部类所实现的功能均可通过非嵌套的普通类实现，但它有利于更好的实现封装。Java源码文件中的主类只能是公开的，假如我们需要对外暴露一个“查询当前是否存在U盘”方法，就要使用系统API注册回调接收USB事件；回调接口实现是一个类，并不需要暴露给外部，此时我们就可以将其作为内部类，并将访问修饰符设为私有。
