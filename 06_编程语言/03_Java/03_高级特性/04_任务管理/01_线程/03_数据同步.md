# 竞态条件
由于多个线程的执行过程具有随机性，当它们都可以访问并修改同一个变量时，可能会出现逻辑错误，这种情况被称为“竞态条件(Race Condition)”，它是破坏“线程安全”的原因之一。
...
测试类中的全局变量 `count` 表示库存商品数量，它是一个静态变量，可以被所有客户线程读写。

客户线程的任务是检查当前商品库存，如果有剩余商品则购买1件，将库存数量减一并且输出到控制台上，如果没有剩余商品，则终止循环结束任务。
...
```text
客户A -> Buy one good, remain count is: 8
