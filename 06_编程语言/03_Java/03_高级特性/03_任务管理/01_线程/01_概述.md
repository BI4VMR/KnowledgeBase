...
本示例通过继承Thread类并重写 `run()` 方法的方式，实现自定义线程。
...
本示例通过构造Thread对象并传入Runnable接口实现的方式，实现自定义线程。
...
## 中断已阻塞的任务
当线程已经处于阻塞状态时，无法主动调用 `isInterrupted()` 方法检查中断状态，此时需要通过异常机制处理中断请求。如果阻塞状态的线程收到中断请求，就会抛出InterruptedException并立刻终止任务，我们可以在线程任务中添加"try ... catch"语句捕获中断异常并进行释放资源等善后操作。
