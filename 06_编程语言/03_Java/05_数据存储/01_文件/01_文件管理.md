...
以目录或文件名称开头的路径被称作相对路径，例如：路径 `dir/test.txt` 表示参考目录内有一个 `dir` 目录，其中有一个 `test.txt` 文件。相对路径本身是不完整的，我们不知道 `dir/test.txt` 指向 `/home/user1/dir/test.txt` 或是 `/home/user2/dir/test.txt` ，参考路径与相对路径拼接后才能组成完整的可寻址路径。

Java的参考路径为程序启动时终端所在的目录(Current Working Directory, CWD)，例如终端在 `/mnt` 目录下执行 `java -jar /opt/app/start.jar` 启动程序，此时程序内读到的默认参考路径为 `/mnt` ，所以我们执行java程序时先将终端切换到程序所在目录。

我们可以在相对路径中使用当前目录符号( `.` )和上层目录符号( `..` )，例如： `./test.txt` 表示当前目录下的文本文件； `../test.txt` 则表示当前目录上层目录中的文本文件。File类不支持解析Shell中的用户目录符号( `~` )，它会被视作文件名称的一部分。

参考路径与Java应用类型有关，使用 `main()` 方法运行的Java Application参考点为当前工程；使用单元测试工具运行的测试类参考点为当前模块；其他类型应用的参考点视具体情况而定。

## 路径分隔符
对于Linux系统路径，我们可以直接使用字符串进行表示；例如路径 `/home/user1/content.txt` 应当写作 `"/home/user1/content.txt"` 。

对于Windows系统路径，反斜杠( `\` )需要使用转义符号( `\\` )表示；例如路径 `C:\Test\content.txt` 应当写作 `"C:\\Test\\content.txt"` 。

File类中的静态变量 `separator` 将在程序启动时被初始化为当前操作系统应当使用的分隔符，因此我们推荐用它拼接路径，以便程序能够跨平台运行。
