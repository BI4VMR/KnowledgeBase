...
对于网络层及层次更低的协议，Java标准库无法提供全功能支持，只能提供获取网卡地址等基本功能，这是因为较低的协议与驱动有关，无法跨平台，如果我们需要直接控制这些协议，需要通过JNI调用驱动与硬件交互或者引入其他第三方库。

Java提供的Socket接口能够支持传输层的TCP和UDP协议，因此也能支持应用层的各种协议，例如HTTP、DNS等。
