...
除了上文示例中展示的方法，NetworkInterface对象还提供了一些方法便于我们获取接口属性，详见下文列表：
...
- `NetworkInterface getParent()` : 判断接口是否为其他接口的子接口。
- `Enumeration<NetworkInterface> getSubInterfaces()` : 判断接口是否为其他接口的子接口。



- `NetworkInterface getByIndex(int index)` : 判断接口是否为其他接口的子接口。
- `NetworkInterface getByName(String name)` : 判断接口是否为其他接口的子接口。
- `NetworkInterface getByInetAddress(InetAddress addr)` : 判断接口是否为其他接口的子接口。
<!-- /TODO -->
