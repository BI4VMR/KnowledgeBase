...
"try"块中的语句将会按照书写顺序执行，一旦遇到错误，程序就会忽略后续语句，并将控制权交给"catch"块，执行其中的异常处理语句。"finally"块中的语句用于执行善后操作，不管是否发生异常都会在最后执行，可以用于关闭I/O流、删除临时文件等，如果我们不需要任何善后操作，也可以省略"finally"块。
