...
# Kotlin - 高级特性 - 任务管理 - 协程 - 与线程相互转换

# 简介
对于回调风格的API，我们可以使用 `suspendCoroutine()` 方法将其转换为挂起函数，以便在协程代码中使用，统一编码风格，减少嵌套层级。

# 回调风格API示例
下文的示例代码展示了一个网络回调接口示例：
...
NetCallback接口定义了两个方法，其中 `onSuccess()` 方法在请求成功时被回调，参数为服务端返回的数据； `onFailure()` 方法则在请求失败时被回调，参数为错误信息。

`request()` 方法用于模拟网络请求，其中开启了一个线程，休眠3秒以模拟网络耗时，休眠结束后根据参数 `result` 的值调用NetCallback实例的方法，向调用者反馈请求结果。

接口定义完毕后，我们就可以调用该接口，传入NetCallback类的实现，并在结果返回后执行对应的操作，例如更新界面等；此处我们只是简单的将回调结果输出到控制台上。
...
`request()` 方法被调用3秒后，NetCallback实现类的 `onSuccess()` 方法被触发，输出了请求结果。

# "suspendCoroutine()"方法

`suspendCoroutine()` 方法用于
...
协程任务运行至 `suspendCoroutine()` 方法时进入挂起状态，随后调度器开始执行 `suspendCoroutine()` 方法内部的语句。当 `request()` 方法回调请求成功的结果时，我们
