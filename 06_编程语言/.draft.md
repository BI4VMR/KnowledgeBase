...
# 挂起函数
在 Kotlin 协程中，delay 函数是一个挂起函数（suspend function）。挂起函数的特点是它们在执行时并不会阻塞调用它们的线程。相反，当遇到挂起函数时，协程会暂停执行，将控制权交还给调用者，从而允许其他任务在这段暂停时间内继续执行。这一机制使得即使在主线程中使用协程和 delay，也不会造成传统意义上由于阻塞而导致的界面冻结或其他性能问题。
具体到您的情况，当在主线程中通过 CoroutineScope 启动一个新的协程，并在这个协程中调用 delay 函数时，主线程不会等待这个延迟时间结束，而是继续执行后续代码（如果有的话）。delay 导致的暂停只影响其所在的协程，而不会影响主线程的事件循环或UI响应（如果是在有UI的应用中）。
因此，使用协程和 delay 可以实现非阻塞的延时操作，提高了程序的响应性和执行效率，特别是在处理IO操作、网络请求或者任何需要异步处理的场景下。



为了便于反复调用，我们可以将前文示例代码中的协程任务定义为一个挂起函数
...
```text
Task1 start. Name:[DefaultDispatcher-worker-1] Time:[16:20:27.548]
Task2 start. Name:[DefaultDispatcher-worker-2] Time:[16:20:27.548]

Task1 end. Name:[DefaultDispatcher-worker-2] Time:[16:20:29.578]
Task2 end. Name:[DefaultDispatcher-worker-2] Time:[16:20:30.568]
```
...
2秒之后，Task2仍处于暂停状态，Task1从暂停状态中恢复，使用先前分配给Task2的工作线程"worker-2"继续执行输出日志动作，该现象说明Task2挂起时已经与工作线程"worker-2"分离，而不会使工作线程阻塞，在此期间工作线程可以处理其他任务。
