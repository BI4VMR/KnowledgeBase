...
opkg remove wpad-basic dnsmasq
rm -rf /tmp/dhcp.leases

opkg install wpad ip-full dnsmasq-full openssh-sftp-server btop iftop curl ncat bash zsh vim-full luci-i18n-base-zh-cn



apk del wpad-basic dnsmasq

apk add --force-overwrite wpad ip-full dnsmasq-full openssh-sftp-server curl bash zsh vim-full luci-i18n-base-zh-cn
...
apk add --allow-untrusted luci-app-openclash-0.47.055.apk


# 查看bandix页面后输入正确用户名与密码也无法登录
...
rm -rf /tmp/hosts/odhcpd

service odhcpd restart


op 25.12.0
2026.01.19~2e5068b9-r1

rm -rf /tmp/dhcp.leases
rm -rf /tmp/odhcpd.lease
...
该页面管理链路层接口，我们需要给br-lan创建子接口，每个子接口对应一个vlan，例如br-lan.10 br-lan.20。


br-lan 配置 网桥 VLAN 过滤

该页面为交换矩阵，配置各vlan与物理端口之间的关系，例如access、trunk等。



网络 - 接口 - 接口

该页面管理网络层接口，我们可以给vlan创建对应的svi，以便使本机可以访问vlan，并提供网关等三层业务，如果仅作交换，不需要三层访问，可以不创建svi

启用vlan之后，原本的lan三层接口可以删除。


注意配置防火墙，新创建的接口不属于任何区域，会导致无法正常通信。


## WLAN 漫游

默认包含wpad-basic包缺少一些功能，所以看不见漫游选项，需要替换

软件包,功能完整度,加密底层库,固件体积占用,适用场景
wpad-basic,基础功能（仅 WPA2/WPA3 个人版）,mbedTLS,最小,闪存较小、仅需普通家庭 Wi-Fi 的路由器
wpad-mbedtls,全功能（含企业认证/漫游/Mesh）,mbedTLS,中等,绝大多数 OpenWrt 用户的首选全功能包
wpad,全功能（含企业认证/漫游/Mesh）,OpenSSL,最大,系统已内置 OpenSSL 或有特定加密兼容需求时
...
# KB5074109
Windows 10
KB5074109导致自定义的文件夹别名失效，更新最新补丁后恢复。
