...
## 命令管理
Shell命令分为两种：一种是Shell内置命令，没有独立的可执行文件；另一种是外置命令，具有独立的可执行文件，我们可以执行 `type <命令名称>` 命令区分一个命令是否为内置命令。
...
|     `%m`      |       主机名       |                       -                        |
|     `%M`      |     完整主机名     |                       -                        |
|     `%l`      |     终端设备名     | 若设备名称以 `/dev/tty` 开头，该部分会被省略。 |
|     `%y`      |     终端设备名     |                       -                        |
|     `%~`      |      工作目录      |     若路径为当前用户的主目录，则显示为 `~`     |
|     `%/`      |      工作目录      |                       -                        |
|     `%D`      |        日期        |       年份为两位的日期（例如：24-12-25）       |
|     `%w`      |        日期        |           星期和日期（例如：周日 8）           |
|     `%t`      |        时间        |          12H HH:MM（例如：1:33下午）           |
|     `%T`      |        时间        |           24H HH:MM （例如：13:33）            |
|     `%#`      |      用户权限      |     管理员显示为 `#` ，普通用户显示为 `%`      |
|     `%j`      |      作业数量      |                       -                        |
|     `%?`      | 前一个命令的返回码 |                       -                        |
|     `%h`      |    历史命令序号    |                       -                        |
...
对于占位符中的 `%` 、 `$` 等符号，Zsh会尝试对其进行解析，如果我们需要显示此类符号，可以在它们之前添加一个 `\` 进行转义，例如： `\%` 将显示为 `%` 、 `\$` 将显示为 `$` 。
...
%(?.%F{#00FF00}√%f.%F{#FF0000}Code:[$?] ×%f)
```

除了行首的主要提示符，Zsh还支持在行尾添加次要提示符，我们可以配置该提示符显示时间、命令执行结果等辅助信息。

次要提示符通过 `RPROMPT` 变量定义，未设置此变量则不显示任何内容，若我们希望启用此功能，可以使用主要提示符的语法进行配置。
