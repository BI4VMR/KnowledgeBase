...
# Shell管理
Shell与常见的应用软件并没有什么不同，它们也以可执行文件作为入口。我们可以查看 `/etc/shells` 配置文件的内容，查询当前系统中已安装的所有Shell以及可执行文件位置。
...
Bash是大多数发行版的默认Shell，其他Shell的基本用法与Bash完全兼容，因此我们必须首先掌握Bash的用法。

## 基本应用
当我们登录至控制台后，屏幕上将会显示“命令提示符”，展示重要的状态信息，此时我们便可以键入命令了。

Redhat系列发行版的命令提示符如下文图片所示：

<div align="center">

![Redhat系列命令提示符](./Assets_终端管理/Bash_Redhat系列命令提示符.jpg)
...
Shell命令分为两种：一种是Shell内置命令，没有独立的可执行文件；另一种是外置命令，具有独立的可执行文件，我们可以执行 `type <命令名称>` 命令查看指定的命令是否为内置命令。

🔴 示例一：区分Shell内置命令与外置命令。
...
Bash允许用户在键入部分字符后，按 `Tab` 键自动补全命令，但默认情况下仅能补全命令本身，无法补全命令的二级菜单与参数等。我们可以安装软件包"bash-completion"，它提供了常用命令的二级菜单与参数补全，能够使Bash更加易用。
