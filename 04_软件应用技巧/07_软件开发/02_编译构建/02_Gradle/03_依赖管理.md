...
如果我们不想指定确切的依赖项版本，可以使用 Maven 风格的版本范围，如 \[1.0, 2.0) 或 (1.0, 2.0]。
...
默认情况下，Gradle 会缓存动态版本和更改模块 24 小时。在此期间，Gradle 不会联系任何已声明的远程仓库以获取新版本。如果您希望 Gradle 更频繁地或在每次执行构建时检查远程仓库，那么您将需要更改生存时间 (TTL) 阈值。
dependencies {
    implementation("org.springframework:spring-web:5.0.3.BUILD-SNAPSHOT")
}

默认情况下，Gradle 会缓存依赖项的更改版本 24 小时。在此时间范围内，Gradle 不会尝试从声明的仓库中解析较新的版本。 阈值可以配置 根据需要，例如，如果您想更早地解析新的快照版本。
...
--offline 命令行开关告诉 Gradle 始终使用缓存中的依赖模块，无论它们是否需要再次检查。离线运行时，Gradle 永远不会尝试访问网络来执行依赖项解析。如果依赖项缓存中不存在所需的模块，构建执行将失败。
刷新依赖关系

您可以从命令行控制不同构建调用的依赖项缓存行为。命令行选项有助于为构建的单次执行做出选择性的临时选择。

有时，Gradle Dependency Cache 可能会与配置仓库的实际状态不同步。也许一个仓库最初配置错误，或者一个“不变”的模块发布不正确。要刷新依赖项缓存中的所有依赖项，请在命令行上使用 --refresh-dependencies 选项。

--refresh-dependencies 选项告诉 Gradle 忽略已解析模块和构件的所有缓存条目。将对所有配置的仓库执行新的解析，重新计算动态版本，刷新模块并下载构件。但是，在可能的情况下，Gradle 会在再次下载之前检查之前下载的构件是否有效。这是通过将仓库中发布的 SHA1 值与现有下载构件的 SHA1 值进行比较来完成的。
<!-- TODOTODOTODO -->
...
| [案例三](#案例三) | 下载依赖组件失败，错误消息为服务端不支持客户端的TLS协议。 |
...
## 案例三
### 问题描述
下载依赖组件失败，错误信息为： `The server may not support the client's requested TLS protocol versions: (TLSv1.2, TLSv1.3)` 。
