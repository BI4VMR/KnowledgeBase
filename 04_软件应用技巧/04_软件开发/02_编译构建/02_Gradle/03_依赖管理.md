...
## 动态版本
### 简介
前文我们设置的版本号都是固定的，某些场景需要动态版本号。

[🧭 【链接名称】](https://docs.gradle.org/current/userguide/single_versions.html)



### 动态版本
有时我们不会确切地制定版本，Gradle提供了以下方案：
...
### 快照版本
在Maven仓库中，版本后缀为"SNAPSHOT"的组件表示开发版本，发布者可能随时更新版本而不提升版本号，使用者每次编译时都应当检查该组件是否有变化。
...
implementation(pateoLibs.ccs.sdk) {
    isChanging = true
}
...
注意：ischarging只对当前依赖组件生效，即使该组件所依赖的其他项产生了变化，Gradle也不会尝试刷新缓存。
...
我们很少使用该选项，这是因为公共仓库中的绝大部分组件都是不可变的，重新下载它们非常耗时。如果我们需要刷新
