...
为了在Gradle模块中引用Maven组件，我们需要编辑该模块的 `build.gradle` 配置文件，按照以下规则声明该组件： `<依赖指令> "<GroupID>:<ArtifactID>:<Version>"` 。
...
下文代码块展示了一个示例工程的目录结构，我们在子模块"app"中建立 `libs` 目录放置库文件：
...
下文代码块展示了一个示例工程的目录结构，"app"模块需要调用"base"和"tool"模块中的公共组件。
...
```text
[root@Fedora Project]# find ~/.gradle/caches/modules-2/files-2.1/ -iname "*<关键词>*" | xargs -t -n 1 rm -rf
```
